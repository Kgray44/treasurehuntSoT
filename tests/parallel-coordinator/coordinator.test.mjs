...
test("priority orders otherwise legal READY candidates while readyAt and PR preserve deterministic ties", () => {
  const early = handoff({ project: "Early", pr: 20, candidateSha: sha("c"), priorityLevel: 5, readyAt: "2026-08-25T10:00:00Z" });
  const urgent = handoff({ project: "Urgent", pr: 30, candidateSha: sha("d"), priorityLevel: 1, paths: ["src/urgent/"] });
  const sameTimeLowerPr = handoff({ project: "Tie", pr: 10, candidateSha: sha("e"), priorityLevel: 5, paths: ["src/tie/"], readyAt: "2026-08-25T10:00:00Z" });
  const plan = coordinate({ handoffs: [early, urgent, sameTimeLowerPr] });
  assert.deepEqual(plan.readyOrder.map((candidate) => candidate.pr), [30, 10, 20]);
});

test("priority cannot bypass explicit dependencies or pre-existing migration serialization", () => {
  const migrationFirst = handoff({ project: "Migration first", pr: 10, candidateSha: sha("c"), migrationFamilies: ["core"] });
  const migrationUrgent = handoff({ project: "Migration urgent", pr: 20, candidateSha: sha("d"), priorityLevel: 1, paths: ["src/other/"], migrationFamilies: ["core"] });
  const prerequisite = handoff({ project: "Prerequisite", pr: 30, candidateSha: sha("e"), priorityLevel: 5, paths: ["src/prerequisite/"] });
  const dependentUrgent = handoff({ project: "Dependent urgent", pr: 40, candidateSha: sha("f"), priorityLevel: 1, paths: ["src/dependent/"], dependencies: [30] });
  const plan = coordinate({ handoffs: [migrationUrgent, dependentUrgent, prerequisite, migrationFirst] });
  assert.deepEqual(plan.readyOrder.map((candidate) => candidate.pr), [10, 30]);
...
test("an ACTIVE urgent project has no READY queue seat", () => {
  const active = handoff({ project: "Active urgent", pr: 1, candidateSha: sha("c"), priorityLevel: 1, status: "ACTIVE" });
  ...
  assert.deepEqual(plan.readyOrder.map((candidate) => [candidate.pr, candidate.seat, candidate.action]), [[2, 1, "FINALIZE_NEXT"]]);
});
...
test("the active window assigns finalization, warm standby, then HOLD", () => {
  ...
  assert.deepEqual(plan.readyOrder.map((candidate) => [candidate.seat, candidate.action]), [
    [1, "FINALIZE_NEXT"],
    [2, "WARM_STANDBY"],
    [3, "HOLD"],
  ]);
...
test("a frozen candidate ignores accumulated movement until promotion into Seat 2", () => {
  ...
  const frozen = coordinate({ handoffs: candidates, changedPathsByPr: { 3: ["src/three/current.ts"], 4: ["src/four/current.ts"] } });
  assert.equal(frozen.candidates.find((candidate) => candidate.pr === 3).action, "HOLD");
  assert.equal(frozen.candidates.find((candidate) => candidate.pr === 3).reasons.includes("RECONCILIATION_REQUIRED"), false);
  const promoted = coordinate({
    handoffs: candidates,
    prStates: { 1: { state: "MERGED" }, 2: { state: "MERGED" } },
    changedPathsByPr: { 4: ["src/four/current.ts"] },
  });
  assert.deepEqual(promoted.readyOrder.map((candidate) => [candidate.pr, candidate.seat, candidate.action]), [
    [3, 1, "FINALIZE_NEXT"],
    [4, 2, "WARM_RECONCILE"],
  ]);
});
...
test("accumulated unrelated movement leaves warm standby alone", () => {
  ...
  const second = handoff({ project: "Second", pr: 2, candidateSha: sha("d"), priorityLevel: 2, paths: ["src/second/"] });
...
test("a warmed standby only has final reconciliation after a materially overlapping preceding merge", () => {
  ...
  const unrelated = evaluateAfterMerge({ handoffs: [first, second], mergeSha: sha("e"), mergedPr: 1, mergedPaths: ["src/unrelated/file.ts"] });
  assert.equal(unrelated.candidates.find((candidate) => candidate.pr === 2).action, "FINALIZE_NEXT");
  const overlapping = evaluateAfterMerge({ handoffs: [first, second], mergeSha: sha("e"), mergedPr: 1, mergedPaths: ["src/second/file.ts"] });
...
test("priority promotion can jump a cold candidate to Seat 1 and request currentness reconciliation", () => {
  ...
  const promoted = handoff({ project: "Promoted", pr: 3, candidateSha: sha("e"), priorityLevel: 1, paths: ["src/promoted/"] });
  const plan = coordinate({ handoffs: [a, b, promoted], changedPathsByPr: { 3: ["src/promoted/current.ts"] } });
  assert.deepEqual(plan.readyOrder.map((candidate) => candidate.pr), [3, 1, 2]);
...
test("priority reordering among frozen candidates leaves them on HOLD", () => {
  ...
  const fourthUrgent = handoff({ project: "Fourth", pr: 4, candidateSha: sha("f"), priorityLevel: 4, paths: ["src/fourth/"] });
  const plan = coordinate({ handoffs: [first, second, third, fourthUrgent], changedPathsByPr: { 3: ["src/third/current.ts"], 4: ["src/fourth/current.ts"] } });
  assert.deepEqual(plan.readyOrder.map((candidate) => candidate.pr), [1, 2, 4, 3]);
...
test("priority changes outside the window do not spend reconciliation work and demotion preserves a warmed candidate", () => {
  ...
  const urgent = handoff({ project: "Urgent", pr: 4, candidateSha: sha("f"), priorityLevel: 1, paths: ["src/urgent/"] });
  const demoted = coordinate({ handoffs: [first, warmed, third, urgent], changedPathsByPr: { 2: ["src/warmed/current.ts"], 3: ["src/third/current.ts"] } });
...
test("a stale live PR head is blocked before it can receive an actionable seat", () => {
  ...
  const plan = coordinate({ handoffs: [stale, fresh], prStates: { 1: { state: "OPEN", headRefOid: sha("e") }, 2: { state: "OPEN", headRefOid: sha("d") } } });
  ...
  assert.deepEqual(plan.readyOrder.map((candidate) => [candidate.pr, candidate.seat]), [[2, 1]]);
});

test("an already-finalizing Seat 1 is not preempted by a later priority change", () => {
  const finalizing = handoff({ project: "Finalizing", pr: 1, candidateSha: sha("c"), priorityLevel: 5, paths: ["src/finalizing/"] });
  const urgent = handoff({ project: "Urgent", pr: 2, candidateSha: sha("d"), priorityLevel: 1, paths: ["src/urgent/"] });
  const plan = coordinate({ handoffs: [finalizing, urgent], finalizingPr: 1, changedPathsByPr: { 1: ["src/finalizing/current.ts"] } });
  assert.deepEqual(plan.readyOrder.map((candidate) => [candidate.pr, candidate.seat, candidate.action]), [
    [1, 1, "FINALIZE_NEXT"],
    [2, 2, "WARM_STANDBY"],
  ]);
});

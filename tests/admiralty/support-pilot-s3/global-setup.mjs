...
export default function globalSetup() {
  const taskRoot = process.env.ADMIRALTY_S3_TASK_ROOT;
  if (!taskRoot) throw new Error("ADMIRALTY_S3_TASK_ROOT is required.");
...
function run(script) {
  ...
  if (result.status !== 0) throw new Error(`ADMIRALTY_SUPPORT_PILOT_S3_FIXTURE_SETUP_FAILED:${result.status ?? 1}`);
}

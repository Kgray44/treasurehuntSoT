...
describe("private durable worker", () => {
  ...
  it("requeues an interrupted owned job for a controlled replacement worker", async () => {
    ...
    await expect(
      dispatchPrivateJobBatch("worker-before-restart", { PRIVATE_ASSET_SCAN: vi.fn() }, { signal: stoppingWorker.signal }),
    ...
    await expect(dispatchPrivateJobBatch("worker-after-restart", { PRIVATE_ASSET_SCAN: resumed })).resolves.toMatchObject({
      claimed: 1,
      processed: 1,
      cancelled: 0,
    });

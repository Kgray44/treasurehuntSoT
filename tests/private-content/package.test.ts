...
function fixture(pathName = "content/tales.json") {
  ...
  return makePayload({
    manifest: {
      ...
      tales: [
        {
          logicalId: "synthetic-tale",
          slug: "clockmaker-lantern",
          title: "The Clockmaker's Lantern",
...
describe("private Chronicle package v1", () => {
  it("authenticates and decrypts a bounded synthetic payload", async () => {
    ...
    await expect(decryptPrivatePackage(packageBytes, "synthetic test passphrase")).resolves.toMatchObject({
      manifest: { packageId: "synthetic-package" },
    });
  });
  it("fails closed for a wrong passphrase and tampering", async () => {
    const packageBytes = await encryptPrivatePayload(fixture(), "synthetic test passphrase");
    await expect(decryptPrivatePackage(packageBytes, "wrong passphrase")).rejects.toMatchObject({
      code: "PRIVATE_PACKAGE_AUTHENTICATION_FAILED",
    });
    const tampered = Buffer.from(packageBytes);
    tampered[tampered.length - 2] ^= 1;
    await expect(decryptPrivatePackage(tampered, "synthetic test passphrase")).rejects.toMatchObject({
      code: "PRIVATE_PACKAGE_AUTHENTICATION_FAILED",
    });
  });
  ...
  it("stages then finalizes only hash-addressed objects outside the repository", async () => {
    const base = await mkdtemp(path.join(tmpdir(), "sealed-hold-test-"));
    const root = path.join(base, "objects-root");
    const stagingRoot = path.join(base, "staging-root");
    try {
      const store = new LocalPrivateAssetStore({ root, stagingRoot });
      const staged = await store.stageObject(Buffer.from("synthetic asset"));
      const stored = await store.finalizeObject(staged);
      expect(stored.storageKey).toMatch(/^objects\/[a-f0-9]{2}\/[a-f0-9]{2}\/[a-f0-9]{64}$/);
      await expect(store.verifyObject(stored.sha256)).resolves.toMatchObject({ exists: true, valid: true });
    } finally {

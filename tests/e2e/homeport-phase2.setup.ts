...
export default async function prepareHomeportPhase2Database() {
  const source = path.resolve(
    process.env.HOMEPORT_PHASE2_SOURCE_DATABASE ??
      "C:/Users/kkids/Documents/Codex_TreasureHunt/prisma/dev.db",
  );
  const target = path.resolve(
    process.env.HOMEPORT_PHASE2_DATABASE_PATH ?? path.join(process.cwd(), ".homeport-phase2-e2e.db"),
  );
  const sourceInfo = await stat(source);
  if (!sourceInfo.isFile() || sourceInfo.size < 1) throw new Error(`Homeport Phase 2 source database is unavailable: ${source}`);

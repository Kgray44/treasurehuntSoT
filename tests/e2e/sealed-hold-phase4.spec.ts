...
test("Phase 4 authenticated protected-media workflow is private, accessible, revocable, and restart-safe", async ({ browser }) => {
  const id = randomUUID();
  ...
  expect((await anonymous.request.get("/api/private-content/media/public/not-a-real-opaque-id?revision=revision-1")).status()).toBe(404);
  ...
  expect(await dispatchPrivateJobBatch(`phase4-worker-a-${id}`, handlers(), { limit: 10, leaseMs: 1_000 })).toMatchObject({
    processed: 1,
  });
  ...
  expect(await dispatchPrivateJobBatch(`phase4-worker-b-${id}`, handlers(), { limit: 10, leaseMs: 1_000 })).toMatchObject({
    processed: 1,
  });

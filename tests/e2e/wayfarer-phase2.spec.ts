...
test.beforeAll(async () => {
  syntheticPng = await sharp({ create: { width: 32, height: 32, channels: 4, background: "#234" } }).png().toBuffer();
});
...
test("Wayfarer Passport persists a private profile, preferences, media, and a simulator identity", async ({ page }) => {
  ...
  const providerPatch = () => page.waitForResponse((response) => new URL(response.url()).pathname === "/api/passport/providers" && response.request().method() === "PATCH" && response.status() === 200);
  let providerUpdate = providerPatch(); await linkedProvider.getByRole("combobox").selectOption("PUBLIC"); await providerUpdate;
  providerUpdate = providerPatch(); await linkedProvider.getByRole("checkbox").click(); await providerUpdate;
  await expect(linkedProvider.getByRole("checkbox")).toBeChecked();
  await page.locator('select[name="PROVIDERS"]').selectOption("PUBLIC"); await page.getByRole("button", { name: "Save privacy controls" }).click();
  await expect(linkedProvider.getByRole("combobox")).toHaveValue("PUBLIC");
  await page.getByRole("button", { name: "Unlink identity" }).click(); await expect(page.getByText("Remove this linked identity?")).toBeVisible();
  await page.getByRole("button", { name: "Confirm unlink identity" }).click(); await expect(page.getByText("Linked identity removed and its token material cleared.")).toBeVisible();
});
...
test("Wayfarer Passport remains operable at required responsive viewports", async ({ page }) => {
  for (const viewport of [{ width: 1440, height: 900 }, { width: 430, height: 932 }, { width: 390, height: 844 }, { width: 844, height: 390 }]) {
    await page.setViewportSize(viewport); await page.goto("/register");

...
test("Wayfarer Personal Harbor loads preferences and persists a profile, privacy rules, and media", async ({
  page,
}) => {
  ...
  await page
    .locator("#profile-avatar-file")
    .setInputFiles({ name: "avatar.png", mimeType: "image/png", buffer: syntheticAvatar });
  ...
  await page
    .locator("#profile-banner-file")
    .setInputFiles({ name: "banner.png", mimeType: "image/png", buffer: syntheticBanner });
  await page.getByRole("button", { name: "Use this crop" }).click();
  ...
  expect({ status: publicProjection.status(), body: await publicProjection.json() }).toMatchObject({
    status: 200,
    body: { handle, displayName: "Isolated Captain", biography: "A private voyage biography.", private: false },
  });

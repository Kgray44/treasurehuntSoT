...
const fail = (message) => { process.stderr.write(`SOUNDING_LINE_ERROR ${message}\n`); process.exitCode = 1; };

async function loadPolicy() {
  const manifest = JSON.parse(await readFile(path.join(policyRoot, "policy-manifest.json"), "utf8"));
  const policy = { manifest };
  for (const file of registryFiles) policy[file.replace(/\.json$/u, "")] = JSON.parse(await readFile(path.join(policyRoot, file), "utf8"));
  policy.digest = sha256(canonicalize(policy));
  return policy;
}

function isSafePath(value) {
  return typeof value === "string" && !path.isAbsolute(value) && !value.includes("..") && !/^[a-z]:/iu.test(value) && !value.includes("\\");
}
...
function scanSensitive(value, label, errors) {
  ...
  for (const [key, child] of Object.entries(value)) {
    if (secretPattern.test(key) && typeof child === "string" && child.length > 0) errors.push(`${label}: secret-like value at ${key}`);
    scanSensitive(child, `${label}.${key}`, errors);
  }
}
function validatePolicy(policy) {
  const errors = [];
  const { ownership, contracts, resources, suites, "impact-map": impact, "release-gates": gates, quarantine, "validation-debt": debt, manifest } = policy;
  ...
  for (const file of registryFiles) if (!manifest.registries.includes(file)) errors.push(`manifest: missing registry ${file}`);
  ...
  for (const owner of ownership.owners) {
    ...
    for (const value of [...owner.sourcePaths, ...owner.testPaths]) if (!isSafePath(value)) errors.push(`owner ${owner.id}: unsafe path ${value}`);
    for (const id of owner.contractIds) if (!contractIds.has(id)) errors.push(`owner ${owner.id}: missing contract ${id}`);
  }
  for (const contract of contracts.contracts) {
    assertKeys(contract, ["id", "name", "authority", "owners", "critical"], `contract ${contract.id}`, errors);
    if (!ownerIds.has(contract.authority)) errors.push(`contract ${contract.id}: missing authority ${contract.authority}`);
    for (const owner of contract.owners) if (!ownerIds.has(owner)) errors.push(`contract ${contract.id}: missing owner ${owner}`);
  }
  for (const suite of suites.suites) {
    assertKeys(suite, ["id", "name", "tier", "owner", "command", "estimatedDuration", "parallelSafe", "resources", "dependencies", "contracts", "affectedPaths", "releaseGates", "currentImplementationState"], `suite ${suite.id}`, errors);
    if (!ownerIds.has(suite.owner)) errors.push(`suite ${suite.id}: missing owner ${suite.owner}`);
    if (!Number.isInteger(suite.tier) || suite.tier < 0 || suite.tier > 7) errors.push(`suite ${suite.id}: invalid tier`);
    for (const id of suite.resources) if (!resourceIds.has(id)) errors.push(`suite ${suite.id}: missing resource ${id}`);
    for (const id of suite.dependencies) if (!suiteIds.has(id)) errors.push(`suite ${suite.id}: missing dependency ${id}`);
    for (const id of suite.contracts) if (!contractIds.has(id)) errors.push(`suite ${suite.id}: missing contract ${id}`);
    for (const id of suite.releaseGates) if (!gateIds.has(id)) errors.push(`suite ${suite.id}: missing gate ${id}`);
    for (const value of suite.affectedPaths) if (!isSafePath(value)) errors.push(`suite ${suite.id}: unsafe affected path ${value}`);
  }
  for (const gate of gates.gates) {
    assertKeys(gate, ["id", "requiredSuites", "conditionalSuites"], `gate ${gate.id}`, errors);
    for (const id of [...gate.requiredSuites, ...gate.conditionalSuites]) if (!suiteIds.has(id)) errors.push(`gate ${gate.id}: missing producer suite ${id}`);
  }
  for (const mapping of [...impact.pathMappings, ...impact.contractMappings]) {
    ...
    for (const id of mapping.contractIds ?? [mapping.contractId]) if (id && !contractIds.has(id)) errors.push(`impact map: missing contract ${id}`);
...
const globRegex = (pattern) => new RegExp(`^${pattern.replace(/[.+^${}()|[\]\\]/gu, "\\$&").replace(/\*\*/gu, "§").replace(/\*/gu, "[^/]*").replace(/§/gu, ".*")}$`, "u");
const matches = (candidate, pattern) => globRegex(pattern).test(candidate);
function registeredForFile(file, suites) { return suites.filter((suite) => suite.affectedPaths.some((pattern) => matches(file, pattern))); }

async function inventory(policy) {
  const files = await walk();
  const vitest = files.filter((file) => /(?:^src\/.*\.test\.(?:ts|tsx)|^tests\/private-content\/.*\.test\.(?:ts|tsx)|^scripts\/features\/.*\.test\.ts)$/u.test(file));
...
function addWithDependencies(selected, reasons, suiteId, reason, suitesById) {
  ...
  for (const dependency of suitesById.get(suiteId).dependencies) addWithDependencies(selected, reasons, dependency, `dependency of ${suiteId}`, suitesById);
}
async function plan(policy, changedPaths, scope) {
  const suitesById = new Map(policy.suites.suites.map((suite) => [suite.id, suite]));
  const selected = new Set(); const reasons = new Map(); let uncertain = false;
  if (scope === "release") {
    for (const suite of policy.suites.suites) addWithDependencies(selected, reasons, suite.id, "release scope is comprehensive", suitesById);
  } else {
    for (const changed of changedPaths) {
      let matched = false;
      for (const mapping of policy["impact-map"].pathMappings) if (matches(changed, mapping.path)) {
        matched = true;
        for (const suiteId of mapping.suiteIds) addWithDependencies(selected, reasons, suiteId, `direct impact path ${changed}`, suitesById);
        for (const contractId of mapping.contractIds) for (const mappingByContract of policy["impact-map"].contractMappings.filter((item) => item.contractId === contractId)) for (const suiteId of mappingByContract.suiteIds) addWithDependencies(selected, reasons, suiteId, `contract expansion ${contractId}`, suitesById);
      }
      for (const owner of policy.ownership.owners) if (owner.sourcePaths.some((pattern) => matches(changed, pattern))) {
        matched = true;
        for (const suite of policy.suites.suites.filter((suite) => suite.owner === owner.id)) addWithDependencies(selected, reasons, suite.id, `owner expansion ${owner.id} for ${changed}`, suitesById);
      }
      if (!matched) uncertain = true;
    }
    if (uncertain) for (const suite of policy.suites.suites) addWithDependencies(selected, reasons, suite.id, "uncertain impact broadening", suitesById);
  }
  const selectedEntries = [...selected].sort().map((id) => ({ suiteId: id, reasons: [...new Set(reasons.get(id))].sort() }));
  const omitted = policy.suites.suites.filter((suite) => !selected.has(suite.id)).map((suite) => ({ suiteId: suite.id, reason: "outside declared impact and no uncertainty" })).sort((a, b) => a.suiteId.localeCompare(b.suiteId));
  const graph = selectedEntries.map(({ suiteId }) => ({ suiteId, dependsOn: suitesById.get(suiteId).dependencies.filter((id) => selected.has(id)).sort() }));
  const sourceFiles = await walk();
  const result = { schemaVersion: "1.0.0", nonAuthoritative: true, execution: "forbidden", policyDigest: policy.digest, sourceDigest: sha256(canonicalize(sourceFiles)), scope, requestedPaths: [...new Set(changedPaths)].sort(), uncertaintyBroadened: uncertain, selected: selectedEntries, omitted, graph };

...
const taskRoot = path.resolve(
  process.env.ADMIRALTY_PHASE1_TASK_ROOT ??
    path.join(required("LOCALAPPDATA"), "ProjectAdmiralty", "sounding-line-phase1"),
);
const databasePath = path.join(taskRoot, "database", "admiralty-phase1.db");
const distDir = process.env.NEXT_DIST_DIR ?? ".next";
const sourceSha = output("git", ["rev-parse", "HEAD"]);
const baseEnv = { ...process.env, ADMIRALTY_PHASE1_TASK_ROOT: taskRoot };
run("scripts/admiralty/prepare-phase1-fixture.mjs", [], baseEnv);
if (!existsSync(databasePath)) throw new Error(`ADMIRALTY_DATABASE_MISSING:${databasePath}`);
const env = {
  ...baseEnv,
  ADMIRALTY_PHASE1_TASK_ROOT: taskRoot,
  ADMIRALTY_PHASE1_DATABASE_PATH: databasePath,
  ADMIRALTY_PHASE1_SOURCE_SHA: sourceSha,

...
const port = Number.parseInt(process.env.ADMIRALTY_PHASE2_WALKTHROUGH_PORT ?? "3794", 10);
const distDir = process.env.ADMIRALTY_PHASE2_WALKTHROUGH_DIST ?? ".next-admiralty-phase2-walkthrough";
const databasePath = path.join(taskRoot, "database", "admiralty-phase2.db");
const credentialHandoffPath = path.join(taskRoot, "credentials", "admiralty-phase2-walkthrough.private.json");
...
if (!/^\.next-admiralty-phase2-[a-z0-9-]+$/u.test(distDir)) throw new Error(`ADMIRALTY_DIST_DIR_REFUSED:${distDir}`);
...
else throw new Error(`ADMIRALTY_WALKTHROUGH_COMMAND_UNKNOWN:${command}`);
...
async function prepare() {
  ...
  run("scripts/admiralty/prepare-phase2-fixture.mjs", [], env);
  run("node_modules/next/dist/bin/next", ["build"], env);
  const preparation = {
    status: "ADMIRALTY_PHASE2_WALKTHROUGH_PREPARED",
    sourceSha,
...
async function start() {
  ...
  if (preparation.distDir !== distDir) throw new Error("ADMIRALTY_WALKTHROUGH_DIST_CHANGED_REPREPARE_REQUIRED");
  ...
  closeSync(stderr);
  const state = {
    status: "ADMIRALTY_PHASE2_WALKTHROUGH_RUNNING",
    pid: child.pid,
    port,
    url: baseUrl,
    sourceSha,
  ...
  return {
    status:
      processAlive && portOwnerPid === pid && health.ok
        ? "ADMIRALTY_PHASE2_WALKTHROUGH_HEALTHY"
        : "ADMIRALTY_PHASE2_WALKTHROUGH_STOPPED",
    pid,
    port,
    portOwnerPid,
    processAlive,
    ...
    fixtureVersion: state?.fixtureVersion ?? "admiralty-phase2-v1",
    credentialHandoffPath,
    statePath,
    distDir,
  };
...
async function stop(report) {
  ...
  if (!state?.pid) {
    if (report)
      process.stdout.write(`${JSON.stringify({ status: "ADMIRALTY_PHASE2_WALKTHROUGH_ALREADY_STOPPED", port })}\n`);
    return;
  }
  const portOwnerPid = ownerPid(port);
  ...
  if (report)
    process.stdout.write(
      `${JSON.stringify({ status: "ADMIRALTY_PHASE2_WALKTHROUGH_STOPPED", pid: state.pid, port })}\n`,
    );
}

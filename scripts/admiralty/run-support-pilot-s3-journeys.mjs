...
if (!taskRoot.startsWith(`${allowedRoot}${path.sep}`)) throw new Error("ADMIRALTY_S3_TASK_ROOT_REFUSED");

const sourceSha = output("git", ["rev-parse", "HEAD"]);
const env = {
  ...process.env,
  ADMIRALTY_S3_TASK_ROOT: taskRoot,
  ADMIRALTY_S3_SOURCE_SHA: sourceSha,
...
run("node_modules/@playwright/test/cli.js", ["test", "-c", "tests/config/playwright/playwright.admiralty-support-pilot-s3.config.ts"], env);
process.stdout.write(
  `${JSON.stringify({ status: "ADMIRALTY_SUPPORT_PILOT_S3_BROWSER_PROOF_COMPLETE", sourceSha, taskRoot })}\n`,
);

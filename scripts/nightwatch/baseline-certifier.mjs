...
export const baselineCheckInventory = Object.freeze([
  ...
  { id: "sounding-line-policy", repairability: "OWNER", dependencies: ["maintenance policy integrity", "workflow admission"] },
  { id: "p34-retirement-ledger", repairability: "AUTO_0", dependencies: ["P34 canonical identities", "retirement ledger"] },
  { id: "active-test-registry", repairability: "AUTO_0", dependencies: ["generated active registry"] },
  { id: "document-index", repairability: "AUTO_0", dependencies: ["Ledgerlight documentation migration records", "documentation index"] },
  { id: "feature-catalog", repairability: "AUTO_0", dependencies: ["Feature Catalog projection"] },
  { id: "deepwater-policy", repairability: "OWNER", dependencies: ["Deepwater/source-policy identity"] },
  ...
  { id: "shared-runtime", repairability: "EXTERNAL", dependencies: ["deterministic shared runtime/dependency prerequisites"] },
]);
...
const generatorCheck = (id, repairability, dependencies, command, args, expectedPaths) => ({
  ...
  dependencies,
  async inspect(context) {
    const changedPaths = async () => (await context.execute("git", ["diff", "--name-only", "--"], context.root)).stdout
      .split(/\r?\n/gu).filter(Boolean).sort();
    const run = async () => {
      const before = await changedPaths();
      await context.execute(command, args, context.root);
      const changed = (await context.execute("git", ["diff", "--name-only", "--", ...expectedPaths], context.root)).stdout
        .split(/\r?\n/gu).filter(Boolean).sort();
      const after = await changedPaths();
      const beforeSet = new Set(before);
      const outside = after.filter((path) => !beforeSet.has(path) && !expectedPaths.includes(path));
      const output = await Promise.all(expectedPaths.map(async (path) => {
        try { return { path, content: await context.readText(path) }; } catch { return { path, content: null }; }
      }));
      return { changed, outside, output };
    };
    const first = await run();
    const second = await run();
    if (canonicalize({ changed: first.changed, output: first.output }) !== canonicalize({ changed: second.changed, output: second.output }))
      throw new Error("GENERATOR_NONDETERMINISTIC");
    if (first.outside.length || second.outside.length) throw new Error(`GENERATOR_SCOPE_ESCAPE:${[...first.outside, ...second.outside].sort().join(",")}`);
...
const defaultChecks = (node) => [
  commandCheck("sounding-line-inventory", "AUTO_0", ["inventory/disposition mapping", "governed test registration"], node, ["scripts/sounding-line/cli.mjs", "inventory", "--completeness"], ({ stdout }) => {
    ...
    if (inventory.completeness?.criticalUnknownCount !== 0 || !String(inventory.completeness?.status ?? "").startsWith("COMPLETE"))
      throw new Error("INVENTORY_INCOMPLETE");
    return { completeness: inventory.completeness };
  }),
  commandCheck("sounding-line-policy", "OWNER", ["maintenance policy integrity", "workflow admission"], node, ["scripts/sounding-line/cli.mjs", "validate-policy"], ({ stdout }) => {
  ...
  generatorCheck("deepwater-policy", "AUTO_0", ["Deepwater/source-policy identity"], node, ["scripts/deepwater/cli.mjs", "audit"], ["Development_Docs/Programs/Deepwater/deepwater-phase-status.json", "Development_Docs/Programs/Deepwater/reports/Project_Deepwater_Phase_5_Governance_Report.md"]),
  {
    id: "migration-inventory", repairability: "OWNER", dependencies: ["migration inventory"],
    async inspect(context) {
      const migrations = (await context.execute("git", ["ls-files", "prisma/mysql-migrations", "prisma/migrations"], context.root)).stdout
        .split(/\r?\n/gu).filter(Boolean).sort();
  ...
  },
  {
    ...
    async inspect(context) {
      ...
      if (!workflow.includes("workflow_dispatch") || !authority.includes("baseline_run_id") || !baseline.includes("nightwatch-baseline-certification"))
  ...
  },
  {
    ...
    async inspect(context) {
      ...
      if (!Number.isInteger(major) || major < 22 || !packageJson.packageManager || !lock) throw new Error("RUNTIME_PREREQUISITE_INVALID");
...
export async function certifyBaseline(options = {}) {
  ...
  for (const check of checks) {
    try {
      performed.push({ id: check.id, status: "PASSED", repairability: check.repairability, dependencies: check.dependencies, detail: await check.inspect(context) });
    } catch (error) {
      const detail = normalizedError(error);
      const failure = { checkId: check.id, fingerprint: fingerprint(check, detail), repairability: check.repairability, detail, dependencies: check.dependencies };
      performed.push({ id: check.id, status: "FAILED", repairability: check.repairability, dependencies: check.dependencies, detail });
      failures.push(failure);
    }
  }
  const classes = new Set(failures.map((failure) => failure.repairability));
  const status = !failures.length ? "CERTIFIED" : classes.has("EXTERNAL") ? "EXTERNAL_BLOCKED" : classes.has("OWNER") ? "OWNER_REQUIRED" : "REPAIR_REQUIRED";
  const deterministicClosureDependencies = [...new Set(failures.flatMap((failure) => failure.dependencies))].sort();
  const recordCore = { schemaVersion: "1.0", protectedMain: { sha: mainSha, treeSha: mainTreeSha }, checks: performed, failures, status, deterministicClosureDependencies };
  return {
    ...
    ...recordCore,
    autoZeroRepairable: failures.filter((failure) => failure.repairability === "AUTO_0").map((failure) => failure.fingerprint),
...
if (process.argv[1]?.endsWith("baseline-certifier.mjs")) {
  ...
  const record = await certifyBaseline({ mainSha: option("--protected-main-sha"), mainTreeSha: option("--protected-main-tree"), now: option("--certified-at") });

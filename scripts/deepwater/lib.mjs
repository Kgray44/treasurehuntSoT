...
export function validateModel({
  ...
  const acceptedCatalogIds = new Set(
    catalog.filter((entry) => entry.status !== "BRANCH_COMPLETE_NOT_MERGED").map((entry) => entry.id),
  );
  ...
  if (reconciliationDocument.mappedEntryCount !== acceptedCatalogIds.size)
    errors.push("Feature Catalog reconciliation does not cover every accepted entry");

...
if (!localAppData) throw new Error("HELM_PHASE3_LOCALAPPDATA_MISSING");
const allowedRoot = path.resolve(localAppData, "ProjectHelm");
const taskRoot = path.resolve(
  process.env.HELM_PHASE3_TASK_ROOT ??
    path.join(allowedRoot, `helm-phase3-browser-${randomUUID().replaceAll("-", "")}`),
);
const databasePath = path.join(taskRoot, "database", "helm-phase3.db");
const canonicalDatabase = path.resolve("C:/Users/kkids/Documents/Codex_TreasureHunt/prisma/dev.db");
const distDir = ".next-helm-phase3-browser";
const distPath = path.resolve(repositoryRoot, distDir);
const journeyPattern = process.env.HELM_PHASE3_JOURNEY_GREP ?? "Give the Orders";

if (!taskRoot.startsWith(`${allowedRoot}${path.sep}`)) throw new Error(`HELM_PHASE3_TASK_ROOT_REFUSED:${taskRoot}`);
if (!databasePath.startsWith(`${taskRoot}${path.sep}`) || databasePath === canonicalDatabase)
  throw new Error(`HELM_PHASE3_DATABASE_REFUSED:${databasePath}`);
if (!distPath.startsWith(`${repositoryRoot}${path.sep}`)) throw new Error(`HELM_PHASE3_DIST_REFUSED:${distPath}`);
if (await exists(databasePath)) throw new Error(`HELM_PHASE3_DATABASE_ALREADY_EXISTS:${databasePath}`);
...
try {
  ...
  const env = {
    ...
    HELM_PHASE3_TASK_ROOT: taskRoot,
    HELM_PHASE3_DATABASE_PATH: databasePath,
    HELM_PHASE3_BROWSER_PORT: String(port),
    SESSION_SECRET: `helm-phase3-${randomUUID()}-task-owned-session-secret`,
  ...
  run(
    ...
    ["test", "-c", "scripts/helm/playwright-phase3.config.ts", "-g", journeyPattern],
    env,
  );

  process.stdout.write(
    `${JSON.stringify({
      status: "HELM_PHASE3_BROWSER_JOURNEY_PASSED",
      sourceSha,
      taskRoot,
      databasePath,
      distDir,
...
function availablePort() {
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      if (!address || typeof address === "string") return reject(new Error("HELM_PHASE3_DYNAMIC_PORT_UNAVAILABLE"));

...
async function main() {
  ...
  const navigationContractPath = path.join(outputRoot, "Project_Homeport_Phase_2_Navigation_Projection_Contract.json");
  ...
  const prettierConfig = (await resolveConfig(modeRegistryPath)) ?? {};
  ...
  type RouteRecord = Readonly<{
    ...
    shellMode: string;
    phase2Implementation?: Readonly<{
      state: string;
      implementationAnchorSha: string;
    }>;
  }>;

  const routeInventory = JSON.parse(readFileSync(routeInventoryPath, "utf8")) as {
    routes: RouteRecord[];
    phase2Implementation?: Readonly<{
      state: string;
      implementationAnchorSha: string;
  ...
  const phase2Validated = routeInventory.phase2Implementation?.state === "VALIDATED";
  const sourceSha =
    routeInventory.phase2Implementation?.implementationAnchorSha ??
  ...
  function modeRecord(route: RouteRecord) {
    ...
    return {
      ...
      phase2Status: phase2Validated
        ? "VALIDATED_WITH_GOVERNED_REPRESENTATIVE_BROWSER_EVIDENCE"
        : "ARCHITECTURE_AND_SOURCE_IMPLEMENTED_PENDING_BROWSER_EVIDENCE",
    };
  }
  ...
  const parityRows = navigationRecords
    ...
    .map((item) => ({
      ...
      account_state: item.anonymousOnly
        ? "ANONYMOUS_OR_ENDED_SESSION"
        : item.authenticatedOnly
          ? "AUTHENTICATED"
          : "ALL",
      destination: item.destination,
      evidence: phase2Validated
        ? item.layer === "GLOBAL"
          ? "HP-P2-EV-G-global-nav-public;HP-P2-EV-K-community-shell"
          : item.layer === "WORKSPACE"
            ? "HP-P2-EV-H-player-navigation;HP-P2-EV-I-captain-navigation;HP-P2-EV-J-creator-navigation;HP-P2-EV-M-workspace-switcher"
            : item.layer === "ACCOUNT"
              ? "HP-P2-EV-E-account-menu-anonymous;HP-P2-EV-F-account-menu-authenticated;HP-P2-EV-R-zoom-account-menu"
              : "HP-P2-EV-N-compact-exit;HP-P2-EV-O-immersive-exit"
        : "PENDING_PHASE_2_BROWSER_EVIDENCE",
      parity_status:
        item.desktopPlacement !== "hidden" && item.mobilePlacement !== "hidden"
          ? "EXACT_FUNCTIONAL_PARITY"
          : "GOVERNED_EXCEPTION",
      exception_rationale:
        item.desktopPlacement !== "hidden" && item.mobilePlacement !== "hidden"
          ? ""
          : "Compatibility alias is never rendered.",
    }));
  ...
  const exitRows = pageRecords
    ...
    .map((record) => ({
      ...
      evidence: phase2Validated
        ...
        : "PENDING_PHASE_2_BROWSER_EVIDENCE",
      status: phase2Validated
        ? "VALIDATED_WITH_REPRESENTATIVE_BROWSER_EVIDENCE"
        : "SOURCE_IMPLEMENTED_PENDING_BROWSER_EVIDENCE",
    }));

  const outputs = new Map<string, string>([
    [modeRegistryPath, await format(JSON.stringify(modeRegistry), { ...prettierConfig, parser: "json" })],
    [navigationContractPath, await format(JSON.stringify(navigationContract), { ...prettierConfig, parser: "json" })],

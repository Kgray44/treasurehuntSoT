...
import { ownerCorrectionRound2ContractIds } from "./phase7-owner-correction-round2-sounding-line-model.mjs";
...
const correctionPaths = [
  "Development_Docs/Projects/Project_Homeport/Project_Homeport_Phase_7_Correction_Round_2_*",
  "Development_Docs/Projects/Project_Homeport/Project_Homeport_Phase_7_Owner_Feedback_Round_2_*",
  "Development_Docs/Projects/Project_Homeport/evidence/phase7-owner-correction-round2/**",
  "Development_Docs/Projects/Project_Homeport/walkthrough/phase7/correction-round2/**",
  "Experience_Images/**",
  "scripts/homeport/*phase7-owner-correction-round2*.mjs",
  "tests/e2e/homeport-phase7-owner-correction-round2.spec.ts",
  "playwright.homeport-phase7-owner-correction-round2.config.ts",
];
const affectedPaths = [
  ...correctionPaths,
...
contracts.status = "phase-7-owner-correction-round-2-pending-owner-rereview";
const existingContracts = new Set(contracts.contracts.map((contract) => contract.id));
for (const id of ownerCorrectionRound2ContractIds) {
...
const names = {
  "unit.homeport": "Project Homeport owner correction Round 2 unit, API, and service contracts",
  "component.homeport": "Project Homeport owner correction Round 2 interaction contracts",
  "browser.homeport": "Project Homeport owner correction Round 2 governed browser evidence",
};
for (const id of suiteIds) {
  const suite = suites.suites.find((candidate) => candidate.id === id);
  if (!suite) throw new Error(`MISSING_HOMEPORT_SUITE:${id}`);
  suite.name = names[id];
  suite.contracts = unique([...suite.contracts, ...ownerCorrectionRound2ContractIds]);
  suite.affectedPaths = unique([...suite.affectedPaths, ...affectedPaths]);
  suite.currentImplementationState = "phase-7-owner-correction-round-2-source-bound";
}
await writeJson("suites.json", suites);

const impactMap = readJson("impact-map.json");
impactMap.status = "phase-7-owner-correction-round-2-impact-map";
for (const pathPattern of correctionPaths) {
  ...
  if (mapping) {
    ...
    mapping.contractIds = unique([...(mapping.contractIds ?? []), ...ownerCorrectionRound2ContractIds]);
  } else {
    impactMap.pathMappings.push({
      path: pathPattern,
      suiteIds: [...suiteIds],
      contractIds: [...ownerCorrectionRound2ContractIds],
    });
  }
}
const mappings = new Map(impactMap.contractMappings.map((mapping) => [mapping.contractId, mapping]));
for (const contractId of ownerCorrectionRound2ContractIds) {
...
process.stdout.write(
  `${JSON.stringify({ status: "HOMEPORT_PHASE7_OWNER_CORRECTION_ROUND2_SOUNDING_LINE_POLICY_UPDATED", contracts: ownerCorrectionRound2ContractIds.length, suites: suiteIds.length, pathMappings: correctionPaths.length })}\n`,
);

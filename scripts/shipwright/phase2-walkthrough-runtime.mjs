...
const credentialHandoffPath = path.join(taskRoot, "credentials", "shipwright-phase2-browser.private.json");
...
if (!databasePath.startsWith(`${taskRoot}${path.sep}`) || databasePath === canonicalDatabase)
  throw new Error(`SHIPWRIGHT_WALKTHROUGH_DATABASE_REFUSED:${databasePath}`);
if (!/^\.next-shipwright-phase2-[a-z0-9-]+$/u.test(distDir)) throw new Error(`SHIPWRIGHT_DIST_DIR_REFUSED:${distDir}`);
if (!Number.isInteger(port) || port < 1024 || port > 65_535) throw new Error(`SHIPWRIGHT_PORT_REFUSED:${port}`);
...
else throw new Error(`SHIPWRIGHT_WALKTHROUGH_COMMAND_UNKNOWN:${command}`);

async function prepare() {
  const existing = await status();
  if (existing.processAlive || existing.portOwnerPid) throw new Error(`SHIPWRIGHT_PORT_OR_PROCESS_BUSY:${port}`);
  const sourceSha = git(["rev-parse", "HEAD"]);
  const env = runtimeEnv(sourceSha);
  run("scripts/shipwright/prepare-phase2-fixture.mjs", [], env);
  run("node_modules/next/dist/bin/next", ["build"], env);
  const preparation = {
    status: "SHIPWRIGHT_PHASE2_WALKTHROUGH_PREPARED",
    sourceSha,
...
async function start() {
  ...
  if (!preparation) throw new Error("SHIPWRIGHT_WALKTHROUGH_NOT_PREPARED");
  const sourceSha = git(["rev-parse", "HEAD"]);
  if (preparation.sourceSha !== sourceSha) throw new Error("SHIPWRIGHT_WALKTHROUGH_SOURCE_CHANGED_REPREPARE_REQUIRED");
  if (preparation.distDir !== distDir) throw new Error("SHIPWRIGHT_WALKTHROUGH_DIST_CHANGED_REPREPARE_REQUIRED");
  if (!existsSync(path.join(repositoryRoot, distDir, "BUILD_ID")))
    throw new Error("SHIPWRIGHT_WALKTHROUGH_PRODUCTION_BUILD_MISSING");
  if (!existsSync(databasePath) || (await stat(databasePath)).size < 1)
    throw new Error("SHIPWRIGHT_WALKTHROUGH_DATABASE_MISSING");
  const existing = await status();
  if (existing.processAlive || existing.portOwnerPid) throw new Error(`SHIPWRIGHT_PORT_OR_PROCESS_BUSY:${port}`);
  ...
  closeSync(stderr);
  const state = {
    status: "SHIPWRIGHT_PHASE2_WALKTHROUGH_RUNNING",
    pid: child.pid,
    port,
    url: baseUrl,
    sourceSha,
  ...
  while (Date.now() < deadline) {
    ...
    if (health.ok) return process.stdout.write(`${JSON.stringify({ ...state, health }, null, 2)}\n`);
    await new Promise((resolve) => setTimeout(resolve, 500));
  }
  throw new Error("SHIPWRIGHT_WALKTHROUGH_HEALTH_TIMEOUT");
}

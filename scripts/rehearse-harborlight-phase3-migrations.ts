...
const databasePath = join(rehearsalRoot, `harborlight-phase3-146-${randomUUID()}.sqlite`);
...
async function main() {
  ...
  try {
    for (const file of await migrations()) {
      for (const statement of splitStatements(await readFile(file, "utf8"))) {
        try {
          ...
          throw new Error(`Migration ${basename(resolve(file, ".."))} failed at ${statement.slice(0, 160)}: ${cause instanceof Error ? cause.message : String(cause)}`);
        }
      }
    ...
    try { await prisma.$executeRawUnsafe("INSERT INTO CommunityVoyageKeepsake (id, ownerAccountId, taleSessionId, safeSnapshot, status, updatedAt) VALUES ('duplicate-legacy', 'owner-legacy', 'legacy-session', '{}', 'READY', CURRENT_TIMESTAMP)"); } catch { legacyUnique = true; }
    try { await prisma.$executeRawUnsafe("INSERT INTO CommunityVoyageKeepsake (id, ownerAccountId, wayfarerKeepsakeId, safeSnapshot, status, updatedAt) VALUES ('duplicate-source', 'owner-source', 'wayfarer-1', '{}', 'READY', CURRENT_TIMESTAMP)"); } catch { sourceUnique = true; }
    ...
    await Promise.all([databasePath, `${databasePath}-journal`, `${databasePath}-wal`, `${databasePath}-shm`].map((file) => rm(file, { force: true })));
  }
}

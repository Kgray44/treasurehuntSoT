...
@{ pid = $process.Id; port = $Port; runtimeRoot = $runtimeRoot; startedAt = (Get-Date).ToString("o"); communityWorkerPid = $null; operationalProjectionPid = $null } | ConvertTo-Json | Set-Content -LiteralPath $statePath -Encoding UTF8

...
- Automated provider lifecycle evidence is synthetic and does not establish a live Google or GitHub authorization
- The supplied staging callbacks target a retained staging runtime that does not contain this OAuth branch, so live external-provider completion remains blocked pending an exact compatible runtime callback

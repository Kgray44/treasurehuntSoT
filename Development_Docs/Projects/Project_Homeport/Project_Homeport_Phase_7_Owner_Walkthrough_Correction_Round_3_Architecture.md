...
|  21 | Postmark webhook policy            | Delivery, bounce, and spam-complaint events use a dedicated HTTP-Basic-authenticated endpoint, validate structure, correlate MessageID, process idempotently, suppress unsafe disclosure, and acknowledge governed retries. Postmark exposes no webhook HMAC contract, so none is fabricated. |

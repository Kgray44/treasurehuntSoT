...
The replacement candidate is frozen for serialized canonical acceptance under tag `project-drydock-phase3-candidate-20260812-r2`. Its exact SHA is bound by that tag and branch head, not by this pre-commit planning text. It may receive exactly one replacement Mainline Decision after acceptance ownership is acquired. Protected integration and exact-main proof remain required after a `RELEASE_GO` decision.

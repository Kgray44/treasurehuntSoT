...
This phase is `QUALIFICATION_IN_PROGRESS_NOT_ACCEPTED` until a new exact candidate is frozen, its candidate-sensitive evidence is rebound, and the current Sounding Line Mainline Decision returns a valid `RELEASE_GO`. Live MySQL and connected external/provider proof remain distinct external evidence and are not represented as passed.

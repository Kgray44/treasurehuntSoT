...
The current implementation also adds deterministic local Library search/filter/ranking, cancellable preview/confirmation, bounded parameter authoring and resolution, target-Chronicle dependency checks after parameter resolution, a Drydock proposed-draft error gate, and a truthful Harborlight installed-package projection. It does not yet satisfy browser qualification, full matrix rehearsal, final documentation/catalog reconciliation, owner acceptance, or protected Mainline Decision obligations.
...
An adapter is presentation-only: it declares owner, version, item kinds, browse/search/picker/summary/inspection projections, selection validation entry point, compatibility, and permissions. It cannot define runtime semantics. The initial adapters consume existing asset/artifact metadata and a read-only Harborlight package/release projection for a clean, current, active installation. Harborlight package items currently preserve logical metadata, not a Shipwright canonical authoring envelope, so the UI exposes release, license, attribution, compatibility, and local-modification state while truthfully marking insertion unavailable. Unaccepted Landfall and Watchglass contracts remain absent, except for a truthful unavailable explanation on an already-existing historical reference.

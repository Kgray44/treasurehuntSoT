...
This roadmap is the implementation sequence that follows the animation-system audit of baseline commit `4dbe8c0ae2fbab2785d1d3f26b8d7ba33bf56aee`. It is a planning artifact, not evidence that any product fix described below has been implemented.
...
This is the exact next implementation phase. It fixes P0 truth and lifecycle failures before any decorative expansion.
...
- Target-integrity and acknowledgment logs contain identifiers and counts, not private payload content.
...
- E2E: chapter release with and without required targets; replay after refresh; replay network log proves no command mutation; reduced-motion semantic order.
...
Begin Phase 1 with a short design record for `SceneTargetContract`, `PresentationOutcome`, mandatory-versus-optional acknowledgment policy, and the player-safe persisted replay source. Freeze those interfaces before parallel target-integrity, WAAPI lifecycle, and replay work begins. Do not start new cinematic decoration, production Rive registration, or platform motion until this truth contract passes its focused gate.

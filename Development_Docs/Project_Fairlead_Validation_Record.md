...
| --------------------------------- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Shared control-plane units        | Passed (8)                    | `node --test tests/github-interaction/github-interaction.test.mjs` (rate modes, credential isolation, cache/304, exhaustion fallback, nine-consumer coalescing, routing, jittered polling, webhook fail-closed behavior, App token lifecycle, GraphQL rate metadata, secondary-limit deferral, stale-lock recovery, and corrupt-cache fail-closed behavior) |
...
| Bridgewatch affected tests        | Passed (16)                   | `bridgewatch/test/github.test.ts`, `bridgewatch/test/config.test.ts`, and `bridgewatch/test/store.test.ts`; GraphQL batching made one GraphQL request and zero per-PR check-run REST calls for two open PRs, while source diagnostics retain REST/GraphQL percentages and bounded telemetry.                                                                |
...
| Governed registry                 | Passed                        | `scripts/sounding-line/test-registry.mjs` registered all nine Fairlead test cases under `unit.sounding-line`.                                                                                                                                                                                                                                               |

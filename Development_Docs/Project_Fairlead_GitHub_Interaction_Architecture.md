...
metrics count requests by transport, cache hits, 304s, coalescing, failures,
...
only configuration/expiry health. The App is a high-volume read plane;
...
`verifyWebhook` is available for a future receiver and fails closed on a
missing, malformed, or mismatched `X-Hub-Signature-256` value. Fairlead does

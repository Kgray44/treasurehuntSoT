...
The local review commands are `sounding-line phase3 governance record-flake`, `record-stale`, and `record-slow`, each supplied a repository-relative redacted record. A qualified flake requires at least two `FAILED_ROOT` observations plus a pass; one retry pass remains `NOT_QUALIFIED`. A protected-contract record is rejected unless it uses `PROTECTED_CONTRACT_REGRESSION_NOT_STALE`. Slow records require an actual duration, budget, and bottleneck rather than silently discarding the sample.

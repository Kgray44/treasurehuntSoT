...
Current pilot evidence is stored outside Git. It includes a lower-trust Phase 2 committed-validation record, a machine Phase 3 unit receipt explicitly invalidated because it was run from a dirty source state, a deterministic impact plan for the `scripts/sounding-line` change, a shard allocation, and a detached-controller status/follow/cancel/clean-terminal lifecycle. The controller pilot is explicitly marked `INVALIDATED_SOURCE_WORKTREE_DIRTY`; it proves only the runtime control path and is not reusable validation evidence. The completion receipt remains pending final clean-source validation.

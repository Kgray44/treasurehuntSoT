...
Phase 4 has reconciled to current `origin/main` at `4edc8de5e30e9748700c19b466061f9b9a97f268` after accepted Admiralty Phase 2 protected-merged PR #28. That phase expands existing FT-B010 through a read-only cross-domain Chartroom and dossier-anchored Support Access; it adds no Feature Catalog denominator entry. Two historical hosted Mainline Decisions failed closed as `EVIDENCE_INVALID`, both solely on `browser.helm`; their finalizers cleaned up and their candidates are historical only. The current product evidence is bound to `5806ccb3d705eb04322a7be1d176d97dd6f2da9e`: Helm route recovery, canceled Player event-stream detachment, and visibility-safe waiting-room recovery. It also accounts for accepted FT-036 Drydock Creator Studio Sea Trials and the accepted Admiralty surface without reopening either owner implementation or closure record. FT-036's catalog fragment remains `BRANCH_COMPLETE_NOT_MERGED` pending its separate owner-controlled record promotion. Homeport evidence remains explicitly carried from `b810e2d0c33cbafb8e4d02c19b9af0db94315783`; Player, Captain, Creator, Community, Restricted Operations, and Bridgewatch have current local proof. Homeport owner re-review remains `PENDING_OWNER_DECISION`; local proof neither records owner or product acceptance nor substitutes for a canonical hosted Mainline Decision. The current reconciled branch is frozen as one candidate and awaits serialized acceptance ownership; Phase 5 remains unauthorized.

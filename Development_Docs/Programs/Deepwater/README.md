...
Phase 4 has been reconciled to current `origin/main` after accepted Bridgewatch Phase 2 added the private FT-035 governed-signal projection and updated the Sounding Line observer wiring. Two historical hosted Mainline Decisions failed closed as `EVIDENCE_INVALID`, both solely on `browser.helm`: first a stalled invitation lookup, then a post-launch Player waiting-room handoff. The current repair is bound to `38038fcb1687a9b2fe4f556fdbfece49796f364d`; it detaches stale Player event streams and rechecks authoritative waiting-room state as visibility settles. The unchanged Homeport journey families remain explicitly paired to `b810e2d0c33cbafb8e4d02c19b9af0db94315783`; Player, Captain, and Bridgewatch are rerun on the current source before any new frozen candidate seeks serialized acceptance ownership. Homeport owner re-review remains `PENDING_OWNER_DECISION`; local proof neither records owner or product acceptance nor substitutes for a canonical hosted Mainline Decision. Phase 5 remains unauthorized.

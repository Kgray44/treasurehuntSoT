...
Phase 4 has been reconciled to current `origin/main` after accepted Bridgewatch Phase 2 added the private FT-035 governed-signal projection and updated the Sounding Line observer wiring. The first replacement candidate then received one canonical hosted Mainline Decision, which failed closed as `EVIDENCE_INVALID` when the Helm invitation journey stalled in its resolving state; it is historical only. The narrow retry repair is bound to `ec564fc632fa4836b6eb0a6f0298815649ac452c`. The unchanged Homeport journey families remain explicitly paired to `b810e2d0c33cbafb8e4d02c19b9af0db94315783`; the changed Captain and Bridgewatch families are rerun on the current source and support one frozen repaired candidate for serialized acceptance ownership. Homeport owner re-review remains `PENDING_OWNER_DECISION`; local proof neither records owner or product acceptance nor substitutes for a canonical hosted Mainline Decision. Phase 5 remains unauthorized.

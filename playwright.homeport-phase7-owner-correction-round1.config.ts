...
const port = Number.parseInt(process.env.HOMEPORT_PHASE7_CORRECTION_PORT ?? "3731", 10);
...
Object.assign(process.env, {
  ...
  PROFILE_MEDIA_ROOT: path.join(taskRoot, "synthetic-media", "profile", `correction-${journeyId}`),
  PRIVATE_CONTENT_ROOT: path.join(taskRoot, "synthetic-media", "private", `correction-${journeyId}`),
});

export default defineConfig({
  testDir: "./tests/e2e",
  testMatch: /homeport-phase7-owner-correction-round1\.spec\.ts/u,
  timeout: 150_000,
  expect: { timeout: 30_000 },
  fullyParallel: false,
  workers: 1,
  retries: 0,
  outputDir: path.join(taskRoot, "traces", `correction-${journeyId}`),
  reporter: [
    ["list"],
    ["html", { outputFolder: path.join(taskRoot, "reports", `playwright-correction-${journeyId}`), open: "never" }],
  ],
  use: {
    ...
    video: journeyId === "S" ? "on" : "retain-on-failure",
  },

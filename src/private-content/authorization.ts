...
export const privateContentAuthorization: PrivateContentAuthorization = {
  async canAdministerPackages() {
    ...
    return session ? { id: session.accountId, kind: "CREATOR" } : null;
  },
  async canReadPrivateAsset({ assetId, playthroughId }) {
    const creator = await requireStudioWorkspace();
        ...
        await db.privateAssetReference.findFirst({
          where: {
            ...
            OR: [
              ...
              ...(creator.account.legacyGameMasterId ? [{ ownerActorId: creator.account.legacyGameMasterId }] : []),
            ],
          },
        }),
      );

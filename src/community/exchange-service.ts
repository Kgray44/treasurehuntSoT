...
import {
  assertPublicationScanStatus,
  verifyCommunityPackage,
  type CommunityPackageFile,
  type CommunityScanStatus,
} from "./package";
...
export async function preflightCommunityPublication(
  ...
  scanStatus: CommunityScanStatus = "SCAN_NOT_CONFIGURED",
) {
  ...
  assertPublicationScanStatus(scanStatus, files);
  return {
    releaseId,
...
export async function persistVerifiedCommunityPackage(
  ...
  const preflight = await preflightCommunityPublication(accountId, releaseId, manifest, files, scanStatus);
  const verified = verifyCommunityPackage(manifest, files);
...
export async function installVerifiedCommunityPackage(
  ...
  const stored = await exchangeDb.communityInstallOperation.create({
    data: {
      id: operation.id,
  ...
  await exchangeDb.communityInstallMapping.createMany({
  ...
  if (operation.status === "COMMITTED")
    await exchangeDb.communityInstallation.create({
      data: {
        accountId: input.accountId,
        packageId: input.packageId,
        releaseId: input.releaseId,

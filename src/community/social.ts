...
function reportId(actor: CommunityActor, idempotencyKey?: string) {
  return idempotencyKey ? `report_${createHash("sha256").update(`${actor.accountId}:${idempotencyKey}`).digest("hex").slice(0, 40)}` : undefined;
}
...
async function resolveSubject(tx: any, subjectType: SocialSubjectType, subjectId: string): Promise<Subject> {
  ...
  if (subjectType === "CREATOR") {
    ...
    return {
      ...
      public: profile.moderationStatus === "ACTIVE" && profile.creatorStatus !== "SUSPENDED" && profile.visibility === "COMMUNITY",
    };
  }
  ...
  if (subjectType === "COLLECTION") {
    ...
    return { type: subjectType, id: collection.id, ownerAccountId: collection.ownerAccountId, public: collection.visibility === "COMMUNITY" };
  }
  ...
  const owner = await tx.communityProfile.findUnique({ where: { id: guide.ownerProfileId }, select: { accountId: true } });
  return {
    type: subjectType,
    id: guide.id,
...
export async function blockAccount(actor: CommunityActor, blockedAccountId: string): Promise<IdempotentOutcome<unknown>> {
  ...
  return socialDb.$transaction(async (tx: any) => {
    ...
    return createUnique(tx, tx.communityBlock, { blockerAccountId_blockedAccountId: { blockerAccountId: actor.accountId, blockedAccountId } }, { blockerAccountId: actor.accountId, blockedAccountId });
  });
}
...
export async function followCreator(actor: CommunityActor, creatorProfileId: string): Promise<IdempotentOutcome<unknown>> {
  ...
  return socialDb.$transaction(async (tx: any) => {
    ...
    return createUnique(tx, tx.communityCreatorFollow, { followerAccountId_creatorProfileId: { followerAccountId: actor.accountId, creatorProfileId } }, { followerAccountId: actor.accountId, creatorProfileId });
  });
}
export async function unfollowCreator(actor: CommunityActor, creatorProfileId: string): Promise<IdempotentOutcome> {
  ...
  return socialDb.$transaction(async (tx: any) =>
    removeUnique(tx, tx.communityCreatorFollow, { followerAccountId_creatorProfileId: { followerAccountId: actor.accountId, creatorProfileId } }),
  );
}
...
export async function saveSubject(actor: CommunityActor, input: { subjectType: SocialSubjectType; subjectId: string; kind?: "SAVE" | "FAVORITE" }): Promise<IdempotentOutcome<unknown>> {
  ...
  return socialDb.$transaction(async (tx: any) => {
    ...
    return createUnique(tx, tx.communitySave, { accountId_subjectType_subjectId_kind: { accountId: actor.accountId, subjectType: input.subjectType, subjectId: input.subjectId, kind } }, { accountId: actor.accountId, subjectType: input.subjectType, subjectId: input.subjectId, kind });
  });
}
export const favoriteSubject = (actor: CommunityActor, subjectType: SocialSubjectType, subjectId: string) => saveSubject(actor, { subjectType, subjectId, kind: "FAVORITE" });
export async function unsaveSubject(actor: CommunityActor, input: { subjectType: SocialSubjectType; subjectId: string; kind?: "SAVE" | "FAVORITE" }): Promise<IdempotentOutcome> {
  socialRate(actor, "unsave", 60);
  const kind = input.kind ?? "SAVE";
  return socialDb.$transaction(async (tx: any) =>
    removeUnique(tx, tx.communitySave, { accountId_subjectType_subjectId_kind: { accountId: actor.accountId, subjectType: input.subjectType, subjectId: input.subjectId, kind } }),
  );
}
export async function activeSaveCount(subjectType: SocialSubjectType, subjectId: string, kind: "SAVE" | "FAVORITE" = "SAVE") {
...
export async function createCollection(actor: CommunityActor, input: { slug: string; title: string; description?: string; visibility?: CollectionVisibility }): Promise<IdempotentOutcome<unknown>> {
  socialRate(actor, "collection-create", 15);
  const slug = normalizeCollectionSlug(input.slug);
  const visibility = input.visibility ?? "PRIVATE";
  if (!isCollectionVisibility(visibility)) fail("COMMUNITY_INVALID_COLLECTION_VISIBILITY", "Unsupported collection visibility.");
  const title = boundedText(input.title, "Collection title", 1, 120);
  const description = input.description === undefined ? null : boundedText(input.description, "Collection description", 0, 2_000, true) || null;
  return socialDb.$transaction(async (tx: any) => {
    await activeProfile(tx, actor);
    const existing = await tx.communityCollection.findUnique({ where: { slug } });
    if (existing) {
      if (existing.ownerAccountId !== actor.accountId) fail("COMMUNITY_COLLECTION_SLUG_TAKEN", "That collection slug is already in use.");
      return { state: "EXISTING" as const, value: existing };
    }
    return { state: "CREATED" as const, value: await tx.communityCollection.create({ data: { ownerAccountId: actor.accountId, slug, title, description, visibility } }) };
  });
}
async function ownCollection(tx: any, actor: CommunityActor, collectionId: string) {
  ...
  if (!collection || collection.ownerAccountId !== actor.accountId) fail("COMMUNITY_ACCESS_DENIED", "You cannot change this collection.");
  return collection;
}
...
export async function addCollectionItem(actor: CommunityActor, input: { collectionId: string; subjectType: SocialSubjectType; subjectId: string }): Promise<IdempotentOutcome<unknown>> {
  ...
  return socialDb.$transaction(async (tx: any) => {
    ...
    const existing = await tx.communityCollectionItem.findUnique({ where: { collectionId_subjectType_subjectId: { collectionId: collection.id, subjectType: input.subjectType, subjectId: input.subjectId } } });
    if (existing) return { state: "EXISTING" as const, value: existing };
    const count = await tx.communityCollectionItem.count({ where: { collectionId: collection.id } });
    if (count >= MAX_COLLECTION_ITEMS) fail("COMMUNITY_COLLECTION_FULL", `Collections are limited to ${MAX_COLLECTION_ITEMS} items.`);
    return { state: "CREATED" as const, value: await tx.communityCollectionItem.create({ data: { collectionId: collection.id, subjectType: input.subjectType, subjectId: input.subjectId, position: count } }) };
  });
}
export async function reorderCollection(actor: CommunityActor, input: { collectionId: string; orderedItemIds: string[]; expectedUpdatedAt: string }): Promise<IdempotentOutcome> {
  ...
  return socialDb.$transaction(async (tx: any) => {
    ...
    const items = await tx.communityCollectionItem.findMany({ where: { collectionId: collection.id }, orderBy: { position: "asc" } });
    if (items.length !== input.orderedItemIds.length || new Set(input.orderedItemIds).size !== items.length || items.some((item: any) => !input.orderedItemIds.includes(item.id)))
...
export function validateReviewInput(input: { rating: number; spoilerFreeBody?: string | null; spoilerBody?: string | null; dimensions?: Record<string, number> }, itemType: string) {
  if (!Number.isInteger(input.rating) || input.rating < 1 || input.rating > 5)
    fail("COMMUNITY_INVALID_RATING", "Review rating must be an integer from 1 to 5.");
  const spoilerFreeBody = input.spoilerFreeBody == null ? null : boundedText(input.spoilerFreeBody, "Spoiler-free review", 10, 5_000);
  const spoilerBody = input.spoilerBody == null ? null : boundedText(input.spoilerBody, "Spoiler review", 0, 5_000, true) || null;
  ...
  if (!allowed && dimensions.length) fail("COMMUNITY_INVALID_REVIEW_DIMENSION", "This item type does not accept review dimensions.");
...
export async function deleteReview(actor: CommunityActor, reviewId: string): Promise<IdempotentOutcome> {
  ...
  return socialDb.$transaction(async (tx: any) => {
    ...
    if (!review || review.authorAccountId !== actor.accountId) fail("COMMUNITY_ACCESS_DENIED", "You cannot delete this review.");
...
export async function revealReviewSpoiler(reviewId: string) {
  ...
  if (!review || !listing || review.status !== "ACTIVE" || review.deletedAt || !review.spoilerBody || listing.publicationStatus !== "PUBLISHED" || !["COMMUNITY", "FEATURED"].includes(listing.visibility))
...
export async function voteReviewHelpful(actor: CommunityActor, reviewId: string): Promise<IdempotentOutcome<unknown>> {
  ...
  return socialDb.$transaction(async (tx: any) => {
    await activeProfile(tx, actor);
    const review = await tx.communityReview.findUnique({ where: { id: reviewId } });
    if (!review || review.status !== "ACTIVE" || review.deletedAt) fail("COMMUNITY_SUBJECT_UNAVAILABLE", "This review cannot receive votes.");
    const listing = await tx.communityListing.findUnique({ where: { id: review.listingId }, include: { owner: { select: { accountId: true } } } });
    if (!listing) fail("COMMUNITY_SUBJECT_UNAVAILABLE", "This review cannot receive votes.");
    if (review.authorAccountId === actor.accountId) fail("COMMUNITY_SELF_HELPFUL_VOTE", "You cannot vote on your own review.");
    await assertNotBlocked(tx, actor.accountId, review.authorAccountId);
    await assertNotBlocked(tx, actor.accountId, listing.owner.accountId);
    return createUnique(tx, tx.communityReviewHelpfulVote, { reviewId_accountId: { reviewId, accountId: actor.accountId } }, { reviewId, accountId: actor.accountId });
  });
}
export async function removeReviewHelpfulVote(actor: CommunityActor, reviewId: string): Promise<IdempotentOutcome> {
  ...
  return socialDb.$transaction(async (tx: any) => removeUnique(tx, tx.communityReviewHelpfulVote, { reviewId_accountId: { reviewId, accountId: actor.accountId } }));
}
...
export async function createComment(actor: CommunityActor, input: { subjectType: "LISTING" | "VOYAGE_LOG" | "GUIDE"; subjectId: string; body: string; spoilerBody?: string | null; parentCommentId?: string; idempotencyKey: string }): Promise<IdempotentOutcome<unknown>> {
  ...
  if (!isCommentSubjectType(input.subjectType)) fail("COMMUNITY_INVALID_COMMENT_SUBJECT", "This subject does not accept comments.");
  ...
  return socialDb.$transaction(async (tx: any) => {
    ...
    const subject = await resolveSubject(tx, input.subjectType, input.subjectId);
    if (!subject.public || !subject.commentsEnabled) fail("COMMUNITY_COMMENTS_DISABLED", "Comments are unavailable for this subject.");
    await assertNotBlocked(tx, actor.accountId, subject.ownerAccountId);
    let depth = 0;
    if (input.parentCommentId) {
      const parent = await tx.communityComment.findUnique({ where: { id: input.parentCommentId } });
      if (!parent || parent.subjectType !== input.subjectType || parent.subjectId !== input.subjectId) fail("COMMUNITY_INVALID_COMMENT_PARENT", "Replies must belong to the same subject.");
      if (parent.deletedAt || parent.status !== "ACTIVE") fail("COMMUNITY_INVALID_COMMENT_PARENT", "Replies require an active parent comment.");
...
export async function revealCommentSpoiler(commentIdValue: string) {
  ...
  if (!comment || comment.status !== "ACTIVE" || comment.deletedAt || !comment.spoilerBody) fail("COMMUNITY_SPOILER_UNAVAILABLE", "This spoiler section is unavailable.");
  const subject = await socialDb.$transaction((tx: any) => resolveSubject(tx, comment.subjectType as SocialSubjectType, comment.subjectId));
...
export async function updateComment(actor: CommunityActor, commentIdValue: string, body: string, spoilerBody?: string | null): Promise<IdempotentOutcome<unknown>> {
  socialRate(actor, "comment-update", 30);
  return socialDb.$transaction(async (tx: any) => {
    const comment = await tx.communityComment.findUnique({ where: { id: commentIdValue } });
    if (!comment || comment.authorAccountId !== actor.accountId || comment.deletedAt) fail("COMMUNITY_ACCESS_DENIED", "You cannot edit this comment.");
    const updated = await tx.communityComment.update({ where: { id: comment.id }, data: { body: validateCommentBody(body), spoilerBody: spoilerBody == null ? null : boundedText(spoilerBody, "Comment spoiler", 0, 5_000, true) || null, spoilerLevel: spoilerBody ? "SPOILER" : "NONE", editedAt: new Date() } });
...
export async function deleteComment(actor: CommunityActor, commentIdValue: string): Promise<IdempotentOutcome> {
  ...
  return socialDb.$transaction(async (tx: any) => {
    const comment = await tx.communityComment.findUnique({ where: { id: commentIdValue } });
    if (!comment || comment.authorAccountId !== actor.accountId) fail("COMMUNITY_ACCESS_DENIED", "You cannot delete this comment.");
...
export async function createReport(actor: CommunityActor, input: { subjectType: string; subjectId: string; reason: string; detail?: string; idempotencyKey?: string }): Promise<IdempotentOutcome<unknown>> {
  ...
  return socialDb.$transaction(async (tx: any) => {
    ...
    const report = await tx.communityReport.create({ data: { ...(id ? { id } : {}), subjectType: requiredId(input.subjectType, "Report subject type"), subjectId: requiredId(input.subjectId, "Report subject"), reporterAccountId: actor.accountId, reason, detail } });
...
export function reportProjection(report: any) {
  ...
  return { id: report.id, subjectType: report.subjectType, subjectId: report.subjectId, reason: report.reason, status: report.status, createdAt: report.createdAt };
}

export const socialLimits = { maxCollectionItems: MAX_COLLECTION_ITEMS, maxCommentDepth: MAX_COMMENT_DEPTH, reviewStatuses: REVIEW_STATUSES } as const;

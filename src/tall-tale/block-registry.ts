...
export const blockRegistry = {
  ...
  visionWaypoint: define({
    ...
    category: "Direction and Location",
    icon: "\u25c9",
    description: "Verify an immutable published waypoint through the governed local Companion runtime.",
    ...
    defaultConfiguration: {
      ...
      assignmentPolicy: {},
      accessibilityPolicy: { activationModes: ["HOLD", "TOGGLE"], reducedMotion: true },
    ...
    fields: [
      ...
      {
        key: "runtimeMode",
        label: "Governed runtime mode",
        kind: "select",
        options: [
          { value: "DEVELOPMENT_MOCK", label: "Development mock" },
          { value: "SHADOW", label: "Shadow · never progresses" },
          { value: "CAPTAIN_CONFIRMED", label: "Captain-confirmed" },
          { value: "AUTOMATIC", label: "Automatic · certification required" },
          { value: "DISABLED", label: "Disabled" },
        ],
        help: "Automatic is rejected unless package certification and real field-evidence gates are complete.",
      },
      {
        key: "scanMode",
      ...
      { key: "captureDurationMs", label: "Capture duration (ms)", kind: "number" },
      { key: "sampleFps", label: "Analysis samples per second", kind: "number" },
      { key: "minimumFrames", label: "Minimum usable frames", kind: "number" },
      text("successEvent", "Story success event", true),
      ...
      { key: "captainFallbackEnabled", label: "Enable Captain fallback", kind: "boolean" },
      {
        ...
        kind: "select",
        options: [
          { value: "CAPTAIN_FALLBACK", label: "Captain fallback" },
          { value: "RETRY_WHEN_ONLINE", label: "Retry when online" },
          { value: "UNAVAILABLE", label: "Unavailable offline" },
        ],
      },
      { key: "assignmentPolicy", label: "Player and crew assignment policy (JSON)", kind: "json" },
      { key: "accessibilityPolicy", label: "Accessibility policy (JSON)", kind: "json" },
    ],
  }),
  arrivalCheck: define({
    type: "arrivalCheck",

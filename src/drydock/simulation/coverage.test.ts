...
const scenario = {
  ...
  title: "Coverage",
  purpose: "Cover both synthetic blocks.",
  seed: "seed",
...
describe("Drydock coverage ledger", () => {
  it("reports exact uncovered paths rather than upgrading partial execution to complete proof", () => {
    ...
    const partial = createDrydockCoverageReport(snapshot, [
      runDrydockScenario(snapshot, { ...scenario, inputs: [{ kind: "CONTINUE" }] }),
    ]);
    expect(partial.proofStatus).toBe("INCOMPLETE_PROOF");
    expect(partial.uncoveredBlockIds).toEqual([]);
  });

  it("turns explicit gaps into unsaved, source-bound Creator suggestions", () => {
    const partial = createDrydockCoverageReport(snapshot, [
      runDrydockScenario(snapshot, { ...scenario, inputs: [{ kind: "CONTINUE" }] }),
    ]);
    expect(createDrydockScenarioSuggestions(partial)).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "UNCOVERED_ENDING",

...
const declaration = (id: string): DrydockVariableDeclaration => ({ schemaVersion: 1, id, name: id, type: { kind: "BOOLEAN" }, scope: "SESSION", allowedOperations: ["assign", "toggle"], privacy: "PLAYER_SAFE" });
...
describe("Drydock bounded static performance analysis", () => {
  it("warns about high fan-out and variable catalog growth before hard limits", () => {
    const blocks = [block("entry", Array.from({ length: 12 }, (_, index) => `target-${index}`)), ...Array.from({ length: 12 }, (_, index) => block(`target-${index}`))];
    const issues = analyzeDrydockPerformance({ blocks, graphAnalysis: analyzeDrydockGraph(blocks), declarations: Array.from({ length: 64 }, (_, index) => declaration(`var-${index}`)) });

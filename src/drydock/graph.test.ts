...
const parseFixture = () => fixture.blocks.map((block) => parseDrydockBlock(block)).filter((result) => result.success).map((result) => result.block);
...
describe("Drydock Phase 2 canonical graph analysis", () => {
  it("proves the complete synthetic Chronicle has one canonical reachable terminal flow", () => {
    ...
    for (let index = 0; index < blocks.length - 1; index++) blocks[index].connections = [{ targetBlockId: blocks[index + 1].id, connectionType: "DEFAULT", orderIndex: 0 }];
  ...
  it("does not pretend a free-form legacy edge condition is statically proven", () => {
    const blocks = structuredClone(parseFixture()).slice(0, 2);
    blocks[0].connections = [{ targetBlockId: blocks[1].id, connectionType: "DEFAULT", conditionExpression: "legacy predicate", orderIndex: 0 }];
    const result = analyzeDrydockGraph(blocks);

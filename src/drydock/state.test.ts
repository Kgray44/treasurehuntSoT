...
describe("Drydock definite-initialization analysis", () => {
  it("requires a write on every predecessor path before a read", () => {
    const graph = buildDrydockChronicleGraph([
      block("entry", ["left", "right"]),
      block("left", ["read"]),
      block("right", ["read"]),
      block("read", []),
    ]);
    const usages: DrydockVariableUsage[] = [
      {
        variableId: "lantern",
        kind: "WRITE",
        blockId: "left",
        fieldPath: "configuration.value",
        operation: "assign",
        privacy: "PLAYER_SAFE",
      },
      {
    ...
    expect(
      analyzeDrydockDefiniteInitialization({ graph, declarations: [declaration], usages }).issues.map(
        (issue) => issue.code,
      ),
    ).toContain("DRYDOCK_VARIABLE_NOT_DEFINITELY_INITIALIZED");
  });
  ...
  it("accepts a declared default as definite initialization at the entry", () => {
    ...
    expect(
      analyzeDrydockDefiniteInitialization({ graph, declarations: [{ ...declaration, defaultValue: false }], usages })
        .issues,
    ).toEqual([]);
  });
  ...
  it("reports state declared or written without any consumer", () => {
    ...
    ).toContain("DRYDOCK_VARIABLE_UNUSED");
    const usages: DrydockVariableUsage[] = [
      {
        variableId: "lantern",
        kind: "WRITE",
    ...
    ).toContain("DRYDOCK_VARIABLE_WRITE_NEVER_READ");
  });
  ...
  it("only classifies a condition constant when no writer can alter its default", () => {
    ...
    expect(
      analyzeDrydockConditionFeasibility({
        blocks: [condition],
        declarations: [{ ...declaration, defaultValue: true }],
        usages: [],
      }).map((issue) => issue.code),
    ).toContain("DRYDOCK_CONDITION_ALWAYS_TRUE");
    const writes: DrydockVariableUsage[] = [
      {
        variableId: "lantern",
        kind: "WRITE",
        blockId: "condition",

...
describe("required Scenario Suite policy", () => {
  it("always requires a baseline path but only derives applicable classes", () => {
    expect(requiredScenarioClasses(snapshot("narrative", "playerConfirmation")).map((item) => item.id)).toEqual([
      "BASELINE_SUCCESS",
    ]);
    expect(requiredScenarioClasses(snapshot("wait", "timer")).map((item) => item.id)).toContain("TIMER_TIMEOUT");
    expect(requiredScenarioClasses(snapshot("captainApproval", "captainManual")).map((item) => item.id)).toContain(

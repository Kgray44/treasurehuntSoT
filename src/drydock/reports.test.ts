...
import { createDrydockValidationReport, creatorReportProjection, diffDrydockReports, isDrydockReportPublicationEligible, supportReportProjection } from "@/drydock/reports";

describe("Drydock source-bound reports", () => {
  const issue = createDrydockIssue({ code: "DRYDOCK_GRAPH_UNREACHABLE", category: "GRAPH", severity: "ERROR", ruleVersion: 1, location: { blockId: "b" }, message: "Private answer must never appear here.", technicalDetail: "secret", remediation: "Connect it." });
  it("binds a deterministic report to immutable source and stable issues", () => {
    const first = createDrydockValidationReport({ source: { b: 1, a: 2 }, issues: [issue], generatedAt: "2026-08-10T00:00:00.000Z" });
    const second = createDrydockValidationReport({ source: { a: 2, b: 1 }, issues: [issue], generatedAt: "2026-08-10T00:00:00.000Z" });
  ...
  it("diffs by stable issue identity and redacts support detail", () => {
    const previous = createDrydockValidationReport({ source: { v: 1 }, issues: [], generatedAt: "2026-08-10T00:00:00.000Z" });
    const next = createDrydockValidationReport({ source: { v: 2 }, issues: [issue], generatedAt: "2026-08-10T00:00:00.000Z" });
  ...
  it("classifies one unambiguous semantic rule move without comparing messages", () => {
    ...
    const previous = createDrydockValidationReport({ source: { v: 1 }, issues: [before], generatedAt: "2026-08-10T00:00:00.000Z" });
    const next = createDrydockValidationReport({ source: { v: 2 }, issues: [after], generatedAt: "2026-08-10T00:00:00.000Z" });
  ...
  it("allows publication only from a complete error-free exact-source receipt", () => {
    const report = createDrydockValidationReport({ source: { v: 1 }, issues: [], generatedAt: "2026-08-10T00:00:00.000Z" });

...
import { createVariableRegistry, type DrydockVariableDeclaration, type DrydockVariableUsage, type DrydockVariableValue } from "@/drydock/variables";
...
const writeInitializes = (usage: DrydockVariableUsage) =>
  usage.kind === "WRITE" && !["increment", "decrement", "toggle", "min", "max", "add", "remove"].includes(usage.operation ?? "assign");

const requiresPriorValue = (usage: DrydockVariableUsage) =>
  usage.kind === "EXPRESSION" ||
  (usage.kind === "WRITE" && ["increment", "decrement", "toggle", "min", "max", "add", "remove"].includes(usage.operation ?? ""));
...
export function analyzeDrydockDefiniteInitialization(input: {
  ...
  const defaults = new Set(input.declarations.filter((declaration) => declaration.defaultValue !== undefined).map((declaration) => declaration.id));
  const usagesByBlock = new Map<string, DrydockVariableUsage[]>();
  for (const usage of input.usages) if (usage.blockId) usagesByBlock.set(usage.blockId, [...(usagesByBlock.get(usage.blockId) ?? []), usage]);
  ...
  while (changed && iterations++ < maximumIterations) {
    changed = false;
    for (const id of input.graph.blocks.keys()) {
      const predecessors = (input.graph.incoming.get(id) ?? []).map((edge) => edge.sourceBlockId).filter((source) => input.graph.blocks.has(source));
      const incoming = id === input.graph.entryBlockId ? new Set(defaults) : intersection(predecessors.map((source) => after.get(source) ?? new Set()));
      ...
      const equal = (left: ReadonlySet<string>, right: ReadonlySet<string>) => left.size === right.size && [...left].every((value) => right.has(value));
  ...
  if (status === "INCOMPLETE_PROOF")
    issues.push(createDrydockIssue({ code: "DRYDOCK_STATE_PROOF_INCOMPLETE", category: "STATE", severity: "WARNING", ruleVersion: 1, location: {}, message: "Definite-initialization analysis reached its explicit proof bound.", remediation: "Reduce the state/graph complexity or request a governed full analysis with a larger bound.", metadata: { maximumIterations } }));
  ...
  for (const declaration of input.declarations) {
    ...
    const read = variableUsages.some((usage) => usage.kind === "READ" || usage.kind === "EXPRESSION" || usage.kind === "OPERATION");
...
export function analyzeDrydockConditionFeasibility(input: {
  ...
  for (const declaration of input.declarations)
    if (declaration.defaultValue !== undefined && !written.has(declaration.id)) state[declaration.id] = declaration.defaultValue;
  const issues: DrydockIssue[] = [];
  for (const block of input.blocks) {
    ...
    try {
      ...
      issues.push(createDrydockIssue({
        ...
        ruleVersion: 1,
        location: { blockId: block.id, blockType: block.blockType, fieldPath: "configuration.expression" },
        message: value ? "This Condition Passage is statically always true." : "This Condition Passage is statically always false.",

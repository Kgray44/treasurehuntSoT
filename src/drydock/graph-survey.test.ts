...
describe("Drydock graph survey", () => {
  it("projects canonical graph facts without exposing authored prose or predicates", () => {
    ...
    source[0].connections = [
      {
        targetBlockId: source[1].id,
        connectionType: "DEFAULT",
        conditionExpression: "private predicate",
        orderIndex: 0,
      },
    ];
    ...
    const survey = createDrydockGraphSurvey(source, undefined, [
      createDrydockIssue({
        ...
        severity: "ERROR",
        ruleVersion: 1,
        location: { blockId: source[0].id },
        message: "not projected",
        remediation: "not projected",
      }),
    ]);

    expect(survey).toMatchObject({
      schemaVersion: 1,
      entryBlockId: source[0].id,
      proofCompleteness: "INCOMPLETE_PROOF",
    });
    expect(survey.edges).toEqual(
      expect.arrayContaining([expect.objectContaining({ hasUnprovenLegacyCondition: true })]),
    );

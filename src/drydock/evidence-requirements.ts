...
const base: readonly DrydockEvidenceRequirement[] = [
  {
    ...
    mandatory: true,
    resolver: "Drydock validation report",
  },
  {
    id: "DD-R-SCENARIOS",
    version: "1",
    capability: "BASELINE",
    requirementType: "SCENARIO_SUITE",
    mandatory: true,
    resolver: "Drydock Scenario Suite",
  },
  {
    id: "DD-R-COMPATIBILITY",
    version: "1",
    capability: "BASELINE",
    requirementType: "COMPATIBILITY",
    mandatory: true,
    resolver: "Drydock compatibility",
  },
];
...
export function deriveDrydockEvidenceRequirements(
  snapshot: PublishedTaleSnapshot,
): readonly DrydockEvidenceRequirement[] {
  const modes = new Set(
    snapshot.chapters
      .flatMap((chapter) => chapter.blocks)
      .map((block) => String(block.completion?.mode ?? "playerConfirmation")),
  );
  ...
  if (modes.has("visionLocation"))
    requirements.push(
      external(
        ...
        "adapter-unavailable",
        "field-evidence",
        "Project Landfall authoritative field-evidence reference",
      ),
    );
  if (modes.has("visionObject"))
    requirements.push(
      external(
        "DD-R-WATCHGLASS-EXTERNAL",
        "OBJECT_PROVIDER",
        "watchglass",
        "adapter-unavailable",
        "provider-evidence",
        "Project Watchglass authoritative evidence reference",
      ),
    );
  if (modes.has("externalWebhook"))
    requirements.push(
      external(
        "DD-R-EXTERNAL-PROVIDER",
        "EXTERNAL_PROVIDER",
        "external-provider",
        "contract-v1",
        "provider-evidence",
        "Registered external-provider evidence reference",
      ),
    );
  ...
  if (hasThreeDimensionalArtifact) {
    requirements.push(
      external(
        "DD-R-ARTIFACT-3D-PERFORMANCE",
        "ARTIFACT_3D",
        "artifact",
        "adapter-unavailable",
        "performance-evidence",
        "Artifact provider performance evidence",
      ),
    );
    requirements.push(
      external(
        "DD-R-ARTIFACT-3D-ACCESSIBILITY",
        "ARTIFACT_3D",
        "artifact",
        "adapter-unavailable",
        "accessibility-evidence",
        "Artifact provider accessibility evidence",
        "ACCESSIBILITY",
      ),
    );
  }
...
export function baseDrydockEvidenceRequirements() {
  return base;
}

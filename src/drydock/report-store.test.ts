...
describe("Drydock durable report store", () => {
  ...
  it("lists only the requested Chronicle's safe receipt metadata", async () => {
    ...
    await expect(listDrydockValidationRuns("tale-a")).resolves.toEqual([
      expect.objectContaining({ id: "record-1", sourceRevision: 7, createdAt: "2026-08-10T00:00:00.000Z" }),
    ]);
    expect(prisma.drydockValidationRun.findMany).toHaveBeenCalledWith(

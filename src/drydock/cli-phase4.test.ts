...
const run = (...args: string[]) =>
  execFileSync(process.execPath, ["--import", "tsx", "src/drydock/phase4-cli.ts", ...args], {
...
describe("Phase 4 Drydock CLI parity", () => {
  ...
  it("evaluates a verified readiness input through the canonical evaluator", () => {
    ...
    const input = {
      ...
      requirements: [],
      requiredSuites: [],
      compatibility: { sourceChecksum: checksum, policyVersion: "v", status: "COMPATIBLE", digest: "d", warnings: [] },
      externalEvidence: [],
  ...
  it("projects only safe immutable publishing evidence", () => {
    const draft = {
      ...
      ruleCatalogVersion: 1,
      validationRunId: "run",
      requiredSuitePolicyVersion: "suite",
      requiredScenarioSuiteIds: [],
      scenarioRunIds: [],
      coverageDigest: "b".repeat(64),
      compatibilityPolicyVersion: "compat",
      compatibilityDigest: "c".repeat(64),
      externalEvidenceDigest: "d".repeat(64),
      waiverIds: [],
      draftDigest: "e".repeat(64),
    };
    const evidence = createDrydockPublishingEvidencePayload({
      draft,
      scenarioRunIds: [],
      coverageDigest: draft.coverageDigest,

...
describe("Drydock evidence requirement registry", () => {
  ...
  it("requires performance and accessibility evidence only for a 3D artifact", () => {
    expect(deriveDrydockEvidenceRequirements(snapshot("playerConfirmation", [{ id: "artifact-1" }])).map((item) => item.id)).not.toContain("DD-R-ARTIFACT-3D-PERFORMANCE");
    expect(deriveDrydockEvidenceRequirements(snapshot("playerConfirmation", [{ id: "artifact-3d", type: "ARTIFACT_3D" }]))).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: "DD-R-ARTIFACT-3D-PERFORMANCE", providerId: "artifact", evidenceKind: "performance-evidence" }),
        expect.objectContaining({ id: "DD-R-ARTIFACT-3D-ACCESSIBILITY", requirementType: "ACCESSIBILITY", evidenceKind: "accessibility-evidence" }),
      ]),
    );
  });
});

...
function effectsForBlock(block: CanonicalDrydockBlock): readonly SideEffect[] {
  ...
  if (
    block.blockType === "artifactReveal" &&
    block.configuration.addToCollection === true &&
    typeof block.configuration.artifactId === "string"
  )
  ...
  if (["chapterComplete", "taleComplete"].includes(block.blockType))
    effects.push({
      kind: "COMPLETION",
      block,
      identity: typeof block.configuration.outcomeId === "string" ? block.configuration.outcomeId : undefined,
    });
...
export function analyzeDrydockSideEffects(input: {
  ...
  const cyclicComponents = input.graphAnalysis.stronglyConnectedComponents.filter(
    (component) =>
      component.length > 1 ||
      (input.graphAnalysis.graph.outgoing.get(component[0]) ?? []).some((edge) => edge.targetBlockId === component[0]),
  );
      ...
      if (effect.kind === "ARTIFACT_GRANT" || effect.kind === "COMPLETION")
        issues.push(
          createDrydockIssue({
            ...
            remediation:
              "Move the effect outside the cycle or add a governed runtime idempotency design before publishing.",
      ...
      for (const duplicate of duplicates)
        issues.push(
          createDrydockIssue({
            ...
            ruleVersion: 1,
            location: {
              blockId: duplicate.block.id,
              blockType: duplicate.block.blockType,
              fieldPath: "configuration.artifactId",
            },
            message: "This Chronicle contains multiple authored grant effects for the same artifact.",
            remediation:
              "Review recipient and repeatability intent; consolidate or add a governed idempotency design where necessary.",
        ...
        issues.push(
          createDrydockIssue({
            ...
            location: {
              blockId: duplicate.block.id,
              blockType: duplicate.block.blockType,
              fieldPath: "configuration.outcomeId",
            },

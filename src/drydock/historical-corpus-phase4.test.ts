    ...
    chapters: [
      {
        ...
        metadata: {},
        orderIndex: 0,
        entryBlockId: fixture.blocks[0]?.id ?? null,
        completionBlockId: null,
        blocks: fixture.blocks.map((block, orderIndex) => ({
          ...block,
          chapterId: "historical-chapter",
          title: `Historical ${block.blockType}`,
          internalLabel: null,
          creatorNotes: null,
          isEnabled: true,
          orderIndex,
...
describe("Phase 4 frozen historical compatibility corpus", () => {
  it("reads and in-memory upcasts every supported Phase 1 block fixture without mutating its published source", () => {
    ...
    expect(assessment.findings.filter((finding) => finding.code === "DRYDOCK_BLOCK_UPCAST_AVAILABLE")).toHaveLength(

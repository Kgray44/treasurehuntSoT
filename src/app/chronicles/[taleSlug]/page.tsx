  ...
  return (
    ...
    >
      <nav className="community-breadcrumbs" aria-label="Chronicle preview location">
        <Link href="/">Gateway</Link><span aria-hidden="true">/</span>
        <Link href="/tales">Explore Chronicles</Link><span aria-hidden="true">/</span>
      ...
      <article className="community-detail chronicle-public-preview">
        ...
        <div className="community-detail__body">
          ...
          <dl className="community-detail__facts">
            <div><dt>Theme</dt><dd>{preview.theme.replaceAll("_", " ").toLocaleLowerCase()}</dd></div>
            <div><dt>Duration</dt><dd>{preview.estimatedDuration ? `${preview.estimatedDuration} minutes` : "Uncharted"}</dd></div>
            <div><dt>Crew</dt><dd>{preview.playerCountMin}–{preview.playerCountMax} Players</dd></div>
            <div><dt>Voyages started</dt><dd>{preview.statistics.voyagesStarted}</dd></div>
            <div><dt>Voyages completed</dt><dd>{preview.statistics.voyagesCompleted}</dd></div>
            <div><dt>Published</dt><dd>{new Date(preview.publishedAt).toLocaleDateString()}</dd></div>
          </dl>
          {preview.contentWarnings ? <section className="community-detail__warning"><h3>Content notes</h3><p>{preview.contentWarnings}</p></section> : null}
          {preview.releaseNotes ? <section><h3>Release notes</h3><p>{preview.releaseNotes}</p></section> : null}
          <section className="community-detail__action" aria-labelledby="chronicle-start-title">
            ...
            <p>Starting enters preparation. This preview has not created a session, Crew, invitation, or participant.</p>
            <Link className="community-button community-button--primary" href={preview.startHref}>Start Chronicle</Link>
          </section>
        </div>
      </article>
      <p className="community-detail-return"><Link href="/tales">Back to Explore Chronicles</Link></p>
    </CommunityPageFrame>
  );
}

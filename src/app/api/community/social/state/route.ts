...
import {
  CommunitySocialStateUnavailable,
  getSocialRelationshipStates,
  socialStateSubjectTypes,
} from "@/community/social-state";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { code: "COMMUNITY_INVALID_INPUT", error: "Social state request is invalid." },
      { status: 400 },
    );
  }

...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(await listModerationEvidence(actor, (await context.params).id));
  } catch (cause) {
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(
      await addModerationEvidence(actor, { ...schema.parse(await request.json()), caseId: (await context.params).id }),
      { status: 201 },
    );
  } catch (cause) {
    return routeError(cause);

...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!validId(id))
    return NextResponse.json(
      { code: "COMMUNITY_INVALID_INPUT", error: "Comment identifier is invalid." },
      { status: 400 },
    );
  return executeSocialMutation(request, commentUpdateInputSchema, (actor, input) =>
    updateComment(actor, id, input.body, input.spoilerBody),
  );
}

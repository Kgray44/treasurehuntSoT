...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!/^[A-Za-z0-9_-]{1,128}$/.test(id)) return NextResponse.json({ code: "COMMUNITY_INVALID_INPUT", error: "Review identifier is invalid." }, { status: 400 });
  return executeSocialMutation(request, creatorResponseInputSchema, (actor, input) => respondToReview(actor, id, input));
}

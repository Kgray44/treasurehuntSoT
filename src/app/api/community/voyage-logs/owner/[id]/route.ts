...
import {
  ...
  publishVoyageLog,
  readOwnedVoyageLog,
  transitionOwnedVoyageLog,
} from "@/community/voyage-log-owner";
import { unavailableWayfarerKeepsakeSource } from "@/community/wayfarer-keepsake-source";
import { requireCanonicalAccountIdentity, verifyPlayerCsrf } from "@/platform/auth";

const update = z
  .object({
    title: z.string(),
    safeSummary: z.string().nullable().optional(),
    visibility: z.enum(["PRIVATE", "CREW_ONLY", "UNLISTED", "COMMUNITY"]),
    spoilerLevel: z.enum(["NONE", "PREVIEW_SAFE", "MINOR", "CHAPTER", "FINALE"]),
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(
      await readOwnedVoyageLog({ ownerAccountId: identity.accountId, voyageLogId: (await params).id }),
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      await transitionOwnedVoyageLog({
        ownerAccountId: identity.accountId,
        voyageLogId: (await params).id,
        to: input.action,
      }),
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(
      await publishVoyageLog({
        ownerAccountId: identity.accountId,
        voyageLogId: (await params).id,
        source: unavailableWayfarerKeepsakeSource,
      }),
      { headers: { "Cache-Control": "private, no-store" } },
    );
  } catch (cause) {

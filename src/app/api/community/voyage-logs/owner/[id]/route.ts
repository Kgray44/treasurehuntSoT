...
import { editVoyageLogDraft, publishVoyageLog, transitionOwnedVoyageLog } from "@/community/voyage-log-owner";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const identity = await requireCanonicalAccountIdentity();
  if (!identity || !(await verifyPlayerCsrf(request.headers.get("x-csrf-token")))) return NextResponse.json({ code: "COMMUNITY_ACCESS_DENIED", error: "The signed-in session could not be verified." }, { status: 403 });
  try { return NextResponse.json(await publishVoyageLog({ ownerAccountId: identity.accountId, voyageLogId: (await params).id, source: unavailableWayfarerKeepsakeSource }), { headers: { "Cache-Control": "private, no-store" } }); }
  catch (cause) { return communityApiError(cause); }
}

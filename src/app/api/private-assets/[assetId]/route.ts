...
export async function GET(request: Request, context: { params: Promise<{ assetId: string }> }) {
  try {
    ...
    if (!(await privateContentAuthorization.canReadPrivateAsset({ assetId, playthroughId })))
    ...
    if (range === false)
      return new Response(null, {
        status: 416,
        headers: { "Content-Range": `bytes */${source.length}`, "Cache-Control": "private, no-store" },
      });

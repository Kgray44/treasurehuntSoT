...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ ok: true, command, preview: await previewAdmiraltyCommand(harborlightModerationPort(operator), command) }, { headers: enforceAdmiraltyRateLimit(`moderation-preview:${operator.accountId}`, 20, 10 * 60_000) });

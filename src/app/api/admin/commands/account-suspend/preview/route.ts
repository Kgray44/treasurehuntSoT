...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { ok: true, command, preview: await previewAdmiraltyCommand(wayfarerAccountSuspendPort(operator), command) },
      { headers: enforceAdmiraltyRateLimit(`account-suspend-preview:${operator.accountId}`, 10, 10 * 60_000) },
    );
  } catch (cause) {

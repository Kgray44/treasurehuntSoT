...
export async function POST(request: Request) {
  ...
  const campaign = await db.campaign.findUnique({ where: { slug: parsed.data.campaignSlug }, include: { playerAccesses: { take: 1 } } });
  if (!campaign || !campaign.playerAccesses[0] || !(await bcrypt.compare(parsed.data.accessCode, campaign.accessCodeHash))) return NextResponse.json({ error: "That invitation could not be recognized." }, { status: 401 });

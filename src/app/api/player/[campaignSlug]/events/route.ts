...
export async function GET(request: Request, context: { params: Promise<{ campaignSlug: string }> }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      const send = (event: ClientProgressEvent) => controller.enqueue(encoder.encode(`id: ${event.sequence}\nevent: progression\ndata: ${JSON.stringify(event)}\n\n`));
      const missed = await db.progressEvent.findMany({ where: { campaignId: access.campaignId, sequence: { gt: Number.isFinite(requested) ? requested : 0 }, releaseAt: { lte: new Date() } }, orderBy: { sequence: "asc" } });
      missed.forEach((event) => send({ id: event.id, type: event.type as ProgressEventType, sequence: event.sequence, payload: JSON.parse(event.payload), releaseAt: event.releaseAt.toISOString() }));
      const channel = `campaign:${access.campaignId}`; eventBus.on(channel, send);

...
export async function GET() {
  ...
  try {
    ...
    try {
      ...
      notice =
        "Some supplementary history could not be refreshed. Your last accepted private records are still shown.";
    }
    const insights = await queryWakebookInsights(session.account.profile.id);
    return NextResponse.json({
      ...insights,
      freshness: notice ? "PARTIAL" : insights.freshness,
      notice,
    });
  } catch {
    return NextResponse.json(
      {
        error:
          "Your private archive insights could not be read safely. Try again.",
      },
      { status: 500 },
    );
  }

...
describe("Drydock Scenario Suite route", () => {
  beforeEach(() => {
    ...
    mocks.checksum.mockReturnValue("a".repeat(64));
  });
  ...
  it("requires Creator authorization and supplies only a server-derived source identity", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/suites", { method: "POST", body: JSON.stringify({ schemaVersion: 1 }) }),
      { params: Promise.resolve({ taleId: "tale" }) },
    );

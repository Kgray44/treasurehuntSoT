...
const unavailable = () => NextResponse.json({ error: "This Chronicle is not available to this Creator account." }, { status: 404, headers: privateHeaders });
...
export async function POST(request: Request, context: { params: Promise<{ taleId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ suite: await saveDrydockScenarioSuite(taleId, await request.json(), checksum) }, { status: 201, headers: privateHeaders });
  } catch (cause) {
    ...
    if (cause instanceof Error && cause.message === "DRYDOCK_SUITE_STALE_SOURCE")
      return NextResponse.json({ error: "This Suite was authored against an older Chronicle source.", code: "DRYDOCK_SUITE_STALE_SOURCE" }, { status: 409, headers: privateHeaders });
    return apiError(cause);
  }
}

...
import { getDrydockScenario } from "@/drydock/scenario-store";
...
export async function GET(request: Request, context: { params: Promise<{ taleId: string; scenarioId: string }> }) {
  ...
  if (revision !== undefined && (!Number.isSafeInteger(revision) || revision < 1))
    return NextResponse.json({ error: "Scenario revision must be a positive integer." }, { status: 400, headers: privateHeaders });
  try {
    const scenario = await getDrydockScenario(taleId, scenarioId, revision);
    if (!scenario) return NextResponse.json({ error: "This Scenario is not available for the current Chronicle." }, { status: 404, headers: privateHeaders });

  ...
  return NextResponse.json(
    {
      ...
      error: "The private package could not be authenticated or opened.",
      correlationId,
    },
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(await inspectPrivatePackage(Buffer.from(body.packageBase64, "base64"), body.passphrase), {
      headers: { "Cache-Control": "private, no-store" },
    });
  } catch (error) {

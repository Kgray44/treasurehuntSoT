...
function failure(error: unknown) {
  ...
  return NextResponse.json({ code: known ? error.code : "PRIVATE_PACKAGE_INVALID", error: "Private upload is unavailable.", correlationId: known ? error.correlationId : crypto.randomUUID() }, { status: known && error.code === "PRIVATE_CONTENT_FORBIDDEN" ? 403 : 400, headers: { "Cache-Control": "private, no-store" } });
}
export async function GET(_request: Request, { params }: { params: Promise<{ uploadId: string }> }) {
  ...
  try { return NextResponse.json(await privateUploadStatus({ actorId: session.userId, uploadId: (await params).uploadId }), { headers: { "Cache-Control": "private, no-store" } }); } catch (error) { return failure(error); }
}

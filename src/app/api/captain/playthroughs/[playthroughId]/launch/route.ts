...
export async function POST(request: Request, context: { params: Promise<{ playthroughId: string }> }) {
  ...
  if (!(await verifyCsrf(session)))
    return NextResponse.json({ error: "Your Captain session expired. Sign in again; Crew access has not changed." }, { status: 403 });
  const parsed = schema.safeParse(await request.json().catch(() => ({})));
  if (!parsed.success)
    return NextResponse.json({ error: "This Voyage cannot begin with the current state. Refresh the Voyage and try again." }, { status: 400 });
  try {

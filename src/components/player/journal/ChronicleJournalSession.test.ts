...
import {
  ...
  ChronicleJournalSession,
} from "./ChronicleJournalSession";
...
vi.mock("@/chronicle/journal-page-model", () => ({
  buildChronicleJournalPages: (...args: unknown[]) => journalPageModelTestDouble.buildPages(...args),
...
describe("ChronicleJournalSession opening outcome consumer", () => {
...
describe("ChronicleJournalSession mounted synchronous teardown", () => {
  beforeEach(() => {
    localStorage.clear();
  ...
  it("cancels its scheduled initial load when unmounted before the queued microtask", async () => {
    ...
    const view = render(
      createElement(StrictMode, null, createElement(ChronicleJournalSession, { sessionId: "microtask" })),
    );
    view.unmount();
  ...
  it("aborts an in-flight initial request in the unmount commit", async () => {
    ...
    const view = render(createElement(ChronicleJournalSession, { sessionId: "pending-request" }));
    await act(flushMicrotasks);
  ...
  it("closes streams, aborts opening work, removes journal keys, and clears reconnect timers for 20 Strict Mode cycles", async () => {
    ...
    for (let cycle = 0; cycle < 20; cycle += 1) {
      ...
      localStorage.setItem(
        `chronicle-journal:${sessionId}`,
        JSON.stringify({ ...readingState(1), openDrawer: "chapters" }),
      );
      const intervalStart = setIntervalSpy.mock.results.length;
      const view = render(createElement(StrictMode, null, createElement(ChronicleJournalSession, { sessionId })));
      await act(flushMicrotasks);
      ...
      const storedBeforeUnmount = localStorage.getItem(`chronicle-journal:${sessionId}`);
      ...
      expect(localStorage.getItem(`chronicle-journal:${sessionId}`)).toBe(storedBeforeUnmount);
      await act(flushMicrotasks);
  ...
  it("treats access revocation as terminal while keeping released pages readable", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const view = render(createElement(ChronicleJournalSession, { sessionId: "revoked" }));
    await act(flushMicrotasks);
    fireEvent.click(screen.getByRole("button", { name: /Open the journal/i }));
    await act(flushMicrotasks);
  ...
  it("keeps an offline transport failure recoverable and distinct from revocation", async () => {
    ...
    render(createElement(ChronicleJournalSession, { sessionId: "offline" }));
    await act(flushMicrotasks);
    fireEvent.click(screen.getByRole("button", { name: /Open the journal/i }));
    await act(flushMicrotasks);
  ...
  it("makes the unopened background inert, transfers focus to the ready journal, and restores drawer focus", async () => {
    ...
    const view = render(createElement(ChronicleJournalSession, { sessionId: "focus" }));
    await act(flushMicrotasks);
  ...
  it("offers manual full and abbreviated replay and restores the replay trigger", async () => {
    ...
    const view = render(createElement(ChronicleJournalSession, { sessionId: "replay" }));
    await act(flushMicrotasks);
    fireEvent.click(screen.getByRole("button", { name: /Open the journal/i }));
    await act(flushMicrotasks);
  ...
  it("opens a completed voyage as a quiet archive without EventSource or reconnect polling", async () => {
    ...
    const view = render(createElement(ChronicleJournalSession, { sessionId: "archive" }));
    await act(flushMicrotasks);
  ...
  it("converges a phase timeout on readable JOURNAL_READY without persisting an aborted first opening", async () => {
    ...
    const view = render(createElement(ChronicleJournalSession, { sessionId: "timeout" }));
    await act(flushMicrotasks);
    ...
    expect(JSON.parse(localStorage.getItem("chronicle-journal:timeout") ?? "{}").hasOpened).not.toBe(true);
  });
  ...
  it("waits at BOOK_SETTLING, then converges a PageFlip readiness timeout without false persistence", async () => {
    ...
    const view = render(createElement(ChronicleJournalSession, { sessionId: "pageflip-timeout" }));
    await act(flushMicrotasks);
    ...
    expect(JSON.parse(localStorage.getItem("chronicle-journal:pageflip-timeout") ?? "{}").hasOpened).not.toBe(true);
  });
  ...
  it("converges a failed PageFlip readiness probe on readable JOURNAL_READY without exposing its error", async () => {
    ...
    const view = render(createElement(ChronicleJournalSession, { sessionId: "pageflip-failure" }));
    await act(flushMicrotasks);
    ...
    expect(JSON.parse(localStorage.getItem("chronicle-journal:pageflip-failure") ?? "{}").hasOpened).not.toBe(true);
  });
  ...
  it("recreates session identity before loading the new cursor, opening policy, stream, and page", async () => {
    ...
    const view = render(createElement(ChronicleJournalSession, { sessionId: "old-identity", identitySession: true }));
    await act(flushMicrotasks);
    ...
    view.rerender(createElement(ChronicleJournalSession, { sessionId: "new-identity", identitySession: true }));
    await act(flushMicrotasks);
  ...
  it.each(["resolve", "reject"] as const)(
    ...
    async (settlement) => {
      ...
      const view = render(createElement(ChronicleJournalSession, { sessionId: "old", identitySession: true }));
      await act(flushMicrotasks);
      expect(identitySignals.get("old")?.aborted).toBe(false);

      view.rerender(createElement(ChronicleJournalSession, { sessionId: "new", identitySession: true }));
      await act(flushMicrotasks);

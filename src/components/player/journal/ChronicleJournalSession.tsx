...
type ChronicleJournalSessionProps = Readonly<{
  sessionId: string;
  identitySession?: boolean;
}>;

export function ChronicleJournalSession({ sessionId, identitySession = false }: ChronicleJournalSessionProps) {
  return (
    <ChronicleJournalSessionIdentity
...
function ChronicleJournalSessionIdentity({ sessionId, identitySession = false }: ChronicleJournalSessionProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      if (identitySession) {
        ...
        const stored = localStorage.getItem(`chronicle-journal:${sessionId}`);
        if (stored) {
          try {
  ...
  const saveReading = useCallback(
    (patch: PlayerJournalReadingStateInput) => {
      ...
      } else localStorage.setItem(`chronicle-journal:${sessionId}`, JSON.stringify(next));
    },
    [identitySession, sessionId, trackedRequest],
  );
  ...
  const pages = useMemo(
    () =>
      state
        ? buildChronicleJournalPages(state.journal, {
...
function JournalStatus({
  ...
  return (
    <main
      className="voyage-shell chronicle-journal-shell journal-status-shell"

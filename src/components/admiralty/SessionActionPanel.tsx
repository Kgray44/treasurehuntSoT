...
export function SessionActionPanel({
  ...
  return (
    <section className="chartroom-support" aria-labelledby="session-action-title">
      ...
      <p>
        Revocation is immediate, cannot expose token material, and records a redacted Wayfarer security event and
        Admiralty receipt.
      </p>
      {notice ? (
      ...
      <label>
        ...
        <select
          ...
          onChange={(event) => {
            ...
            setIdempotencyKey(`session_${crypto.randomUUID().replaceAll("-", "")}`);
            setPreview(null);
            setReceipt(null);
          }}
        >
          {active.map((session) => (
            <option key={session.id} value={session.id}>
              {session.deviceLabel ?? "Device not recorded"} ({session.sessionType})
            </option>
          ))}
        </select>
      </label>
      <label>
        ...
        <textarea
          ...
          placeholder="Describe why the session must be revoked."
        />
      </label>
      <label>
      ...
      <button
        type="button"
        disabled={busy || reason.trim().length < 8 || !sessionId}
        onClick={() =>
          void run(async () => {
      ...
      {receipt ? (
        <p role="status">
          ...
          {receipt.resultSummary.alreadyRevoked

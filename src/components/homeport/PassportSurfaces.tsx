...
type PassportOverviewDto = {
  sections: Array<{ id: string; label: string; href: string; count: number; empty: string }>;
};
export function PassportHome() {
  const { state, reload } = useResource<PassportOverviewDto>("/api/passport/overview");
    ...
    <div className="harbor-stack">
      ...
      <div className="passport-card-grid">
        {state.value.sections.map((section) => (
          <Link href={section.href} key={section.id} className="passport-card">
            <span>{section.count}</span>
            <h2>{section.label}</h2>
            <p>{section.count ? "Open this part of your record." : section.empty}</p>
            <strong>
      ...
      <section className="harbor-callout">
        <strong>Profile and Passport are related, but not the same.</strong>
        <p>
          Your public Profile is the identity other people may see. Your Chronicle Passport is the private, record-led
          home for your own Voyage history.
        </p>
        <Link href="/account/profile">Edit public Profile</Link>
      </section>
    </div>
  );
}
...
type HistoryItem = {
  id: string;
  chronicle: { title: string; cover: string | null; versionChecksum: string };
...
type HistoryDto = {
  items: HistoryItem[];
  invitations: Array<{ id: string; chronicleTitle: string; lifecycleStatus: string }>;
  nextCursor: string | null;
};
export function HistoryExplorer() {
  const [search, setSearch] = useState("");
  const [query, setQuery] = useState("");
  const resource = useResource<HistoryDto>(
    `/api/passport/history${query ? `?search=${encodeURIComponent(query)}` : ""}`,
  );
  if (resource.state.status === "loading") return <LoadingState label="Reading Chronicle History" />;
  if (resource.state.status === "error") return <ErrorState message={resource.state.message} retry={resource.reload} />;
  return (
    <div className="harbor-stack">
      <form
        className="harbor-search"
        role="search"
        onSubmit={(event) => {
      ...
      >
        <label htmlFor="history-search">Search your history</label>
        <div>
          <input
            id="history-search"
            value={search}
      ...
      {resource.state.value.items.length ? (
        <ol className="history-list">
          {resource.state.value.items.map((item) => (
            <li key={item.id}>
              <article>
                <div>
                  <span className="harbor-badge">{item.lifecycleStatus}</span>
                  <h2>{item.chronicle.title}</h2>
                  <p>
                    {item.timestamps.completedAt
                      ? `Completed ${new Date(item.timestamps.completedAt).toLocaleDateString()}`
        ...
        <div className="harbor-empty harbor-empty--large">
          <h2>No Chronicle history found</h2>
          <p>
            {query
              ? "No owner-authorized records match that search."
              : "Completed and terminal Voyage records will appear here without publishing their private details."}
          </p>
          {query && (
            <button
              className="button"
      ...
      {resource.state.value.invitations.length > 0 && (
        <section className="harbor-panel">
          <h2>Invitation history</h2>
          <ul>
            {resource.state.value.invitations.map((item) => (
              <li key={item.id}>
                {item.chronicleTitle} · {item.lifecycleStatus}
              </li>
            ))}
          </ul>
        </section>
...
type HistoryDetailDto = HistoryItem & {
  participant: { name: string; role: string; crewRole: string | null };
  timing: Record<string, { seconds?: number | null; accuracy?: string } | string>;
  completedChapters: Array<{ chapterId: string; title: string; completedAt: string }>;
  choiceSummary: Array<{ reason: string }>;
  reflection: { privateNote: string | null } | null;
  memories: Array<{ id: string; title: string; body: string | null; createdAt: string }>;
  keepsake: { status: string; generatedAt: string; participantCount: number } | null;
  provenance?: unknown;
};
export function HistoryDetail({ recordId }: { recordId: string }) {
  const resource = useResource<HistoryDetailDto>(`/api/passport/history/${encodeURIComponent(recordId)}`);
  ...
  if (resource.state.status === "loading") return <LoadingState label="Opening Chronicle record" />;
  if (resource.state.status === "error") return <ErrorState message={resource.state.message} retry={resource.reload} />;
  ...
  return (
    <div className="harbor-stack">
      <Link href="/passport/history">← Chronicle History</Link>
      <section className="passport-record-hero">
        ...
        <dl className="harbor-definition-list">
          <div>
            <dt>Participant</dt>
            <dd>
              {value.participant.name} · {value.participant.role}
            </dd>
          </div>
          <div>
      ...
      <section className="harbor-panel">
        ...
        {value.completedChapters.length ? (
          <ol>
            {value.completedChapters.map((chapter) => (
              <li key={chapter.chapterId}>
                {chapter.title} · {new Date(chapter.completedAt).toLocaleDateString()}
              </li>
            ))}
          </ol>
        ) : (
          <p className="harbor-empty">No exact completed-chapter events are available for this pinned version.</p>
        )}
        {value.choiceSummary.map((summary, index) => (
          <p className="harbor-field-hint" key={index}>
            {summary.reason}
          </p>
        ))}
      </section>
      <form
        className="harbor-panel harbor-form"
        onSubmit={(event) => {
          event.preventDefault();
          void mutate(`/api/passport/history/${encodeURIComponent(recordId)}`, "PATCH", { privateNote: note || null });
        }}
      >
      ...
      <form
        ...
        onSubmit={async (event) => {
          event.preventDefault();
          const ok = await mutate(`/api/passport/history/${encodeURIComponent(recordId)}/memories`, "POST", memory);
          if (ok) setMemory({ title: "", body: "" });
        }}
      >
        <h2>Add Chronicle Memory</h2>
        <label>
          Title
          <input
            maxLength={120}
      ...
      <section className="harbor-panel">
        <h2>Private Keepsake</h2>
        {value.keepsake ? (
          <>
            <p>
              Status: {value.keepsake.status}. Generated {new Date(value.keepsake.generatedAt).toLocaleString()}.
            </p>
          ...
          <>
            <p>A Keepsake is private until explicit participant consent permits scoped details.</p>
            <button
              type="button"
              className="button"
...
export function MemoriesExplorer() {
  ...
  const remove = async (item: MemoryDto) => {
    ...
    try {
      await responseBody(
        await fetch(
          `/api/passport/history/${encodeURIComponent(item.recordId)}/memories/${encodeURIComponent(item.id)}`,
      ...
      setMessage(cause instanceof Error ? cause.message : "Memory could not be deleted.");
      setError(true);
    }
  };
  return (
...
type ArtifactListDto = {
  ...
  assemblies: Array<unknown>;
  achievements: Array<unknown>;
};
export function ArtifactExplorer() {
  const [search, setSearch] = useState("");
  const [query, setQuery] = useState("");
  const resource = useResource<ArtifactListDto>(
    `/api/passport/artifacts${query ? `?search=${encodeURIComponent(query)}` : ""}`,
  );
  if (resource.state.status === "loading") return <LoadingState label="Opening Artifact Cabinet" />;
  if (resource.state.status === "error") return <ErrorState message={resource.state.message} retry={resource.reload} />;
  return (
    <div className="harbor-stack">
      <form
      ...
      {resource.state.value.items.length ? (
              ...
              <article className="artifact-card">
                ...
                <p>
                  {item.status === "UNRESOLVED"
                    ? "Witnessed in shared inventory; ownership remains unresolved."
                    : "Backed by an authoritative grant record."}
                </p>
                <Link className="button" href={`/passport/artifacts/${encodeURIComponent(item.id)}`}>
                  View provenance
                </Link>
              </article>
            </li>
        ...
        <div className="harbor-empty harbor-empty--large">
          ...
          <p>
            {query
              ? "No owner-authorized artifacts match that search."
              : "Artifacts appear only after authoritative grant or explicit witnessed-state materialization."}
          </p>
        </div>
      )}
    </div>
...
type ArtifactDetailDto = {
  id: string;
  artifact: { id: string; name: string; type: string; representation: string; accessibleRepresentation: string };
  provenance: {
    chronicle: string;
  ...
  personalization: { favorite: boolean; privateNote: string | null; visibility: string; archived: boolean };
  displayCases: Array<{ id: string; name: string }>;
  assemblies: Array<{ id: string; name: string; status: string }>;
};
export function ArtifactDetail({ artifactId }: { artifactId: string }) {
  const resource = useResource<ArtifactDetailDto>(`/api/passport/artifacts/${encodeURIComponent(artifactId)}`);
  const { csrfToken, setDirty } = usePersonalHarbor();
  const [draft, setDraft] = useState<ArtifactDetailDto["personalization"] | null>(null);
  const [message, setMessage] = useState("");
  const [error, setError] = useState(false);
  useEffect(() => {
  ...
  useEffect(() => () => setDirty(false), [setDirty]);
  if (resource.state.status === "error") return <ErrorState message={resource.state.message} retry={resource.reload} />;
  if (resource.state.status === "loading" || !draft) return <LoadingState label="Opening artifact provenance" />;
  const value = resource.state.value;
  const save = async (event: FormEvent) => {
    ...
    try {
      await responseBody(
        await fetch(`/api/passport/artifacts/${encodeURIComponent(artifactId)}`, {
          method: "PATCH",
          headers: { "content-type": "application/json", "x-csrf-token": csrfToken },
          body: JSON.stringify({
      ...
      setDirty(false);
      setMessage("Artifact personalization saved.");
      resource.reload();
    } catch (cause) {
      setMessage(cause instanceof Error ? cause.message : "Artifact could not be saved.");
      setError(true);
    }
  };
  return (
    <div className="harbor-stack">
      <Link href="/passport/artifacts">← Artifact Cabinet</Link>
      ...
      <section className="harbor-panel">
        ...
        <dl className="harbor-definition-list">
          <div>
            <dt>Chronicle</dt>
            <dd>{value.provenance.chronicle}</dd>
          </div>
          <div>
            <dt>Record status</dt>
        ...
        {value.provenance.status === "UNRESOLVED" && (
          <div className="harbor-callout">
            <strong>Witnessed, not claimed as owned.</strong>
            <p>Shared inventory evidence is preserved without silently converting it into an ArtifactGrantReceipt.</p>
          </div>
        )}
      </section>
      <form className="harbor-panel harbor-form" onSubmit={save}>
        <h2>Private personalization</h2>
        ...
        <label className="harbor-checkbox">
          ...
          />
          Archive in my cabinet
        </label>
        <button className="button button--primary">Save personalization</button>
        <MutationMessage message={message} error={error} />
      </form>
    </div>
  );
...
export function SavedExplorer() {
  const resource = useResource<{ items: SavedItem[] }>("/api/passport/saved");
  const { csrfToken } = usePersonalHarbor();
  const [message, setMessage] = useState("");
  const [error, setError] = useState(false);
  if (resource.state.status === "loading") return <LoadingState label="Reading saved Community items" />;
  if (resource.state.status === "error") return <ErrorState message={resource.state.message} retry={resource.reload} />;
  const unsave = async (item: SavedItem) => {
    ...
    try {
      await responseBody(
        await fetch("/api/community/social/unsave", {
          method: "POST",
          headers: { "content-type": "application/json", "x-csrf-token": csrfToken },
      ...
      setMessage(cause instanceof Error ? cause.message : "Saved item could not be removed.");
      setError(true);
    }
  };
  return (
    <div className="harbor-stack">
      <section className="harbor-callout">
        <strong>Eligibility is checked each time this page opens.</strong>
        <p>
          Private, unlisted, blocked, quarantined, archived, removed, or missing subjects are omitted without revealing
          why. Raw save rows are never returned.
        </p>
      </section>
      {resource.state.value.items.length ? (
        <ol className="saved-grid">
          {resource.state.value.items.map((item) => (
            <li key={`${item.subjectType}:${item.subjectId}`}>
              <article className="harbor-panel">
                <span className="harbor-badge">{item.subjectType.replaceAll("_", " ")}</span>
                <h2>{item.title}</h2>
                {item.summary && <p>{item.summary}</p>}
                <p className="harbor-field-hint">Saved {new Date(item.savedAt).toLocaleDateString()}</p>
                <div className="personal-harbor__actions">
                  <Link className="button" href={item.href}>
                    Open in Community
                  </Link>
                  <button type="button" className="button button--quiet" onClick={() => void unsave(item)}>
                    Remove saved item
                  </button>
                </div>
              </article>
        ...
        <div className="harbor-empty harbor-empty--large">
          <h2>No eligible saved items</h2>
          <p>Save a public item in Community Harbor to return to it here.</p>
          <Link className="button" href="/community">
            Explore Community Harbor
          </Link>
        </div>
      )}
      <MutationMessage message={message} error={error} />

...
function storageRoot() {
  const configured = process.env.CHRONICLE_ASSET_ROOT;
  if (configured) {
    if (!path.isAbsolute(configured)) throw new Error("Asset storage is not configured correctly.");
    return path.normalize(configured);
  }
  return path.join(process.cwd(), ".data", "chronicle-assets");
}
...
export async function ingestAsset(taleId: string, file: File, userId: string, replaceAssetId?: string) {
  ...
  const maximum = Number(process.env.CHRONICLE_MAX_UPLOAD_MB ?? 25) * 1024 * 1024;
  ...
  if (!replaceAssetId) {
    ...
    if (duplicate) {
      logger.info(
        { area: "chronicle-assets", taleId, assetId: duplicate.id },

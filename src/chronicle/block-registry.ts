...
export const blockRegistry = {
  ...
  artifactReveal: define({
    ...
    fields: [
      ...
      { key: "selectedRecipientProfileIds", label: "Selected recipient profile IDs (JSON)", kind: "json" },
      { key: "requiredCrewRole", label: "Required crew role", kind: "text" },
      { key: "discoveringMembershipId", label: "Discovering membership ID", kind: "text" },
      {
        key: "personalGrantState",
        label: "Personal grant state",
        kind: "select",
        options: ["COLLECTED", "ENTRUSTED", "ASSEMBLY_COMPONENT"].map((value) => ({ value, label: value })),
      },
      { key: "assemblyDefinitionId", label: "Assembly definition ID", kind: "text" },
      { key: "componentRole", label: "Assembly component role", kind: "text" },
      {
        key: "receiptState",
        label: "Receipt state",
        kind: "select",
        options: ["ACTIVE", "REVOKED"].map((value) => ({ value, label: value })),
      },
      { key: "correctionOfGrantId", label: "Corrected grant receipt ID", kind: "text" },
      { key: "correctionReason", label: "Correction or revocation reason", kind: "text" },
      completion(),
    ],
  }),

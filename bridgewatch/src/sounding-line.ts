...
export const nodeStates = ["QUEUED", "RUNNING", "PASSED", "PASSED_AFTER_RETRY", "FAILED", "BLOCKED", "CANCELLED", "NOT_RUN", "SKIPPED", "UNKNOWN"] as const;
...
const projectionSchema = z.object({
  ...
  workers: z.array(z.object({ id: z.string(), runId: z.string(), lane: z.string(), state: z.literal("RUNNING"), heartbeatAt: z.string().nullable() }).strict()),
  plans: z.array(z.object({
    id: z.string(), sourceSha: z.string().nullable(), gate: z.string(), state: z.string(), createdAt: z.string().nullable(), cleanupState: z.enum(["CLEAN", "PENDING", "FAILED", "UNKNOWN"]), finalDecision: z.string().nullable(),
    nodes: z.array(z.object({ id: z.string(), suiteId: z.string(), state: z.enum(nodeStates), queuedAt: z.string().nullable(), startedAt: z.string().nullable(), completedAt: z.string().nullable(), attempt: z.number().int().positive(), rootFailureId: z.string().nullable() }).strict()),
  }).strict()),
}).strict();
...
export function testTotals(projection: SoundingLineProjection | null) {
  ...
  const roots = new Set(nodes.filter((node) => node.state === "FAILED" && node.rootFailureId).map((node) => node.rootFailureId));
  return {
...
export class SoundingLineCollector {
  ...
  cached(): SoundingLineProjection | null { return this.store.get<SoundingLineProjection>("sounding-line:projection")?.value ?? null; }
  async refresh(): Promise<SoundingLineProjection | null> {
    ...
    try {
      const args = this.config.BRIDGEWATCH_SOUNDING_LINE_PROJECTION_PATH ? [script, this.config.BRIDGEWATCH_SOUNDING_LINE_PROJECTION_PATH] : [script];
      const { stdout } = await execFileAsync(process.execPath, args, { timeout: this.config.BRIDGEWATCH_REQUEST_TIMEOUT_MS, maxBuffer: 256 * 1024 });

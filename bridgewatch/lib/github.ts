...
export class GithubCollector {
  ...
  cached(): Snapshot | null {
    return this.store.get<Snapshot>("github:snapshot")?.value ?? null;
  }
  ...
  async refresh(): Promise<Snapshot | null> {
    ...
    try {
      const [repo, pulls, runs] = await Promise.all([
        this.get<{ default_branch?: string }>("repo", base),
        this.get<unknown[]>("pulls", `${base}/pulls?state=open&per_page=50`),
        this.get<{ workflow_runs?: unknown[] }>("runs", `${base}/actions/runs?per_page=20`),
      ]);
  ...
  private async get<T>(key: string, url: string): Promise<T> {
    ...
    try {
      const headers: Record<string, string> = {
        ...
        "User-Agent": "bridgewatch-phase1/0.1",
      };

...
export function buildServer() {
  ...
  const app = Fastify({ logger: { redact: ["req.headers.authorization", "req.url"] }, bodyLimit: config.BRIDGEWATCH_TELEMETRY_MAX_BODY_BYTES });
  ...
  app.addHook("onRequest", async (request, reply) => {
    ...
    const isOperationalEndpoint = pathname === "/healthz" || pathname === "/readyz" || pathname.startsWith("/api/telemetry/");
    if (isOperationalEndpoint || !config.BRIDGEWATCH_DASHBOARD_USERNAME || !config.BRIDGEWATCH_DASHBOARD_PASSWORD) return;
    const expected = Buffer.from(`${config.BRIDGEWATCH_DASHBOARD_USERNAME}:${config.BRIDGEWATCH_DASHBOARD_PASSWORD}`).toString("base64");
  ...
  const summary = () => {
    ...
    const workers = store.workers().map((worker) => ({ ...worker, effectiveState: workerState(worker, config.BRIDGEWATCH_TELEMETRY_STALE_MS) }));
    ...
    const reporterObservedAt = workers.length
      ? workers.reduce((latest, worker) => Date.parse(worker.heartbeatAt) > Date.parse(latest) ? worker.heartbeatAt : latest, workers[0]!.heartbeatAt)
      : null;
    const reporterState = workers.length === 0 ? "UNMEASURED" : workers.some((worker) => worker.effectiveState === "STALE") ? "STALE" : "FRESH";
    const attention = [
      ...(snapshot ? [] : [{ level: "ACTION", code: "GITHUB_UNAVAILABLE", message: "No GitHub cache is available yet." }]),
      ...workers.filter((worker) => worker.effectiveState === "BLOCKED").map((worker) => ({ level: "BLOCKED", projectId: worker.project, code: "WORKER_BLOCKED", message: `${worker.workerId} reports BLOCKED activity.` })),
      ...workers.filter((worker) => worker.effectiveState === "STALE").map((worker) => ({ level: "NOTICE", projectId: worker.project, code: "WORKER_STALE", message: `${worker.workerId} has a stale heartbeat.` })),
      ...rootAttention(soundingLineProjection),
      ...(soundingLineProjection?.plans.filter((plan) => plan.finalDecision === "RELEASE_NO_GO" || plan.finalDecision === "EVIDENCE_INVALID").map((plan) => ({ level: "BLOCKED", code: "SOUNDING_LINE_DECISION", message: `${plan.id} reports ${plan.finalDecision}.` })) ?? []),
    ];
    return {
      ...
      sources: [
        ...
        { name: "sounding-line", state: soundingLineProjection ? "FRESH" : "UNAVAILABLE", observedAt: soundingLineProjection?.observedAt ?? null },
  ...
  const acceptsTelemetry = (request: { headers: { authorization?: string }; query: Record<string, unknown>; ip: string }, reply: { code: (code: number) => { send: (body: unknown) => unknown } }) => {
    if (Object.keys(request.query).some((key) => /token|authorization/iu.test(key))) return reply.code(400).send({ error: "Telemetry credentials are header-only" });
    if (!authorizeTelemetry(request.headers.authorization, config.BRIDGEWATCH_TELEMETRY_TOKEN)) return reply.code(401).send({ error: "Unauthorized" });
    const now = Date.now();
  ...
  app.post<{ Body: unknown; Querystring: Record<string, unknown> }>("/api/telemetry/heartbeat", async (request, reply) => {
...
function rootAttention(projection: ReturnType<SoundingLineCollector["cached"]>) {
  ...
  for (const node of projection.plans.flatMap((plan) => plan.nodes)) {
    if (node.state === "BLOCKED" && node.rootFailureId) roots.set(node.rootFailureId, (roots.get(node.rootFailureId) ?? 0) + 1);
    if (node.state === "FAILED") roots.set(node.rootFailureId ?? node.id, roots.get(node.rootFailureId ?? node.id) ?? 0);
  }
  return [...roots].map(([id, blocked]) => ({ level: "BLOCKED", code: "ROOT_FAILURE", message: `${id}: 1 root failure${blocked ? `; ${blocked} blocked dependents` : ""}.` }));
}
...
function programTotals(
  ...
  return {
    projects: { total: projects.length, complete: projects.filter((project) => project.state === "COMPLETE").length, active: projects.filter((project) => ["ACTIVE", "TESTING", "REVIEW", "WAITING", "STALE", "MERGED"].includes(project.state)).length, planned: projects.filter((project) => project.state === "PLANNED").length, blocked: projects.filter((project) => project.state === "BLOCKED").length, externalPending: projects.filter((project) => project.state === "EXTERNAL_PENDING").length },
    phases: { total: phases.length, completeOrMerged: phases.filter((phase) => ["COMPLETE", "MERGED"].includes(phase.state)).length, active: phases.filter((phase) => ["ACTIVE", "TESTING", "REVIEW", "WAITING", "STALE"].includes(phase.state)).length, planned: phases.filter((phase) => phase.state === "PLANNED").length, blocked: phases.filter((phase) => phase.state === "BLOCKED").length },
    milestones: { completed: milestones.filter((milestone) => milestone.state === "ACCEPTED").length, total: milestones.length },
    operational: { openPullRequests, activeWorkers: workers.filter((worker) => ["WORKING", "TESTING"].includes(worker.effectiveState)).length, activeTestNodes: tests.running, rootFailures: tests.rootFailures },
  };
}

...
describe("representative Phase 2 operational load", () => {
  it.each([10, 25, 50])("accepts %i authenticated activity heartbeats and keeps warm summary under one second", async (count) => {
    ...
    try {
      ...
      for (let index = 0; index < count; index += 1) {
        const response = await app.inject({ method: "POST", url: "/api/telemetry/heartbeat", headers: { authorization: "Bearer performance-token" }, payload: { workerId: `worker-${index}`, project: "bridgewatch", phase: "2", task: "Performance fixture", state: "WORKING", branch: "codex/project-bridgewatch-phase2-wire-the-signals", sourceSha: "236c27241bb8d1630274f5d5412ec9addbdb8893", host: "performance-host", startedAt: timestamp, heartbeatAt: timestamp } });

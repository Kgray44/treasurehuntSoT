...
const port = Number.parseInt(process.env.HOMEPORT_PHASE7_CORRECTION_PORT ?? "3762", 10);
...
export default defineConfig({
  testDir: "./tests/e2e",
  testMatch: /homeport-phase7-owner-correction-round3\.spec\.ts/u,
  timeout: 180_000,
  expect: { timeout: 30_000 },
  fullyParallel: false,
  workers: 1,
  retries: 0,
  outputDir: path.join(taskRoot, "traces", `round3-${journeyId}`),
  reporter: [
    ["list"],
    ["html", { outputFolder: path.join(taskRoot, "reports", `playwright-round3-${journeyId}`), open: "never" }],
  ],
  use: {
    ...
    video: ["N", "O", "P", "Q"].includes(journeyId) ? "on" : "retain-on-failure",
  },
  projects: [{ name: `homeport-phase7-owner-correction-round3-${journeyId}`, use: { browserName: "chromium" } }],
  webServer: {

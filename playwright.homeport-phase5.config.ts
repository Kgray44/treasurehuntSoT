...
const port = Number.parseInt(process.env.HOMEPORT_PHASE5_PORT ?? "3515", 10);
...
const databasePath = path.resolve(path.join(taskRoot, "database", "phase5-e2e.db"));
...
Object.assign(process.env, {
  ...
  HOMEPORT_PHASE5_SOURCE_DATABASE: sourceDatabasePath,
  HOMEPORT_PHASE5_DATABASE_PATH: databasePath,
  HOMEPORT_PHASE5_EVIDENCE_ROOT: evidenceRoot,
  PROFILE_MEDIA_ROOT: profileMediaRoot,
  PRIVATE_CONTENT_ROOT: privateContentRoot,
});

/** Phase 5 owns its copied database, port, media, browser output, fixture secrets, and evidence. */
export default defineConfig({
  testDir: "./tests/e2e",
  testMatch: /homeport-phase5\.spec\.ts/u,
  globalSetup: "./tests/e2e/homeport-phase5.setup.ts",
  timeout: 900_000,
  ...
  projects: [{ name: "homeport-phase5", use: { browserName: "chromium" } }],
  webServer: {

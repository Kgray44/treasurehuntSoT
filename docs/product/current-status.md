...
Feature Catalog, Project Tideglass Phases 1-3, and the completed Lanternwake system join identity, role-aware
...
**Project Tideglass Phases 1-3 are available on main:** exact immutable
Chronicle edition comparison and its semantic intelligence now have a
discoverable Chronicle-detail and Passport-history **What changed?** journey.
The route respects server-derived audiences, exact played-edition anchors,
multiple historical records, spoilers, compatibility and accessibility changes,
partial and historical states, and safe return context. Creator Studio
published-version comparison uses the same `CREATOR_FULL` Tideglass semantic
projection, not a raw snapshot diff. Candidate and integrated-main Sounding Line
evidence passed with clean release records. This is source integration, not
deployment, live-provider behavior, or production MySQL execution.

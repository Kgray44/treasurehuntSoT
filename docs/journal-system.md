...
`journal-page-model.ts` builds stable title, edition, chapter, block, and endpaper pages from the released projection. Current progress remains in the version-pinned `TaleSession`, `RevealState`, inventory, verification records, and ordered events. Per-Player page position, map selection/zoom, artifact selection, message selection/read IDs, text scale, and presentation preferences are stored separately in `PlayerProfile.preferences`; browsing any section never moves the story. Completed journals use the same released projection from the pinned immutable edition, disable mutations, and retain the checksum/edition record.
...
Chapters keeps the book dominant. Map uses a full chart, released markers, legend, zoom, and selected-location detail. Artifacts uses a scalable gallery plus inspection panel. Messages uses a history column and readable letter panel with persistent read state. The old right-side section buttons and essential drawers are removed. The horizontal parchment/brass tab row scrolls at narrow widths, remains separate from the objective/status region, supports Arrow/Home/End keyboard navigation, and drives real URLs. Page-turn controls, actions, text scaling, live announcements, focus-visible states, route focus transfer, and reduced-motion behavior remain keyboard and assistive-technology accessible.
